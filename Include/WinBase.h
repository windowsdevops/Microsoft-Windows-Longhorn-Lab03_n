...
#define FILE_FLAG_MUI_REDIRECT           0x00040000
#define FILE_FLAG_MUI_FALLBACK           0x00020000
#define FILE_FLAG_COPY_TEMPLATE_METADATA 0x00010000
...
#define COPY_FILE_COPY_LINK                   0x00000800
...
InitializeConditionVariable (
    IN PCONDITION_VARIABLE ConditionVariable
    );
...
typedef struct _JIT_DEBUG_INFO32 {
    DWORD dwSize;
    DWORD dwProcessorArchitecture;
    DWORD dwThreadID;
    ...
    DWORD ContextRecord;
} JIT_DEBUG_INFO32, *LPJIT_DEBUG_INFO32;

typedef struct _JIT_DEBUG_INFO64 {
    DWORD dwSize;
    DWORD dwProcessorArchitecture;
    DWORD dwThreadID;
    DWORD dwReservedAlignment;
    DWORD64 ExceptionAddress;
    DWORD64 ExceptionRecord;
    DWORD64 ContextRecord;
} JIT_DEBUG_INFO64, *LPJIT_DEBUG_INFO64;
...
#define OF_MUI_REDIRECT     0x00010000
#define OF_MUI_FALLBACK     0x00020000
...
InterlockedCompareExchangePointer (
    ...
    __in __opt PVOID ExChange,
    __in __opt PVOID Comperand
    );
...
#define InterlockedIncrementLongPtr                 InterlockedIncrement64
#define InterlockedDecrementLongPtr                 InterlockedDecrement64
#define InterlockedExchangeLongPtr                  InterlockedExchange64
#define InterlockedExchangeAddLongPtr               InterlockedExchangeAdd64
#define InterlockedCompareExchangeAcquireLongPtr    InterlockedCompareExchangeAcquire64
#define InterlockedCompareExchangeReleaseLongPtr    InterlockedCompareExchangeRelease64
#define InterlockedAndLongPtr                       InterlockedAnd64
#define InterlockedOrLongPtr                        InterlockedOr64
#define InterlockedXorLongPtr                       InterlockedXor64

#else

#define InterlockedIncrementLongPtr                 InterlockedIncrement
#define InterlockedDecrementLongPtr                 InterlockedDecrement
#define InterlockedExchangeLongPtr                  InterlockedExchange
#define InterlockedExchangeAddLongPtr               InterlockedExchangeAdd
#define InterlockedCompareExchangeLongPtr           InterlockedCompareExchange
#define InterlockedCompareExchangeAcquireLongPtr    InterlockedCompareExchangeAcquire
#define InterlockedCompareExchangeReleaseLongPtr    InterlockedCompareExchangeRelease
#define InterlockedAndLongPtr                       InterlockedAnd
#define InterlockedOrLongPtr                        InterlockedOr
#define InterlockedXorLongPtr                       InterlockedXor
...
GetShortPathNameA(
    ...
    __out_ecount(cchBuffer) LPSTR  lpszShortPath,
...
GetShortPathNameW(
    ...
    __out_ecount(cchBuffer) LPWSTR  lpszShortPath,
    __in DWORD    cchBuffer
    );
#ifdef UNICODE
...
GetLongPathNameA(
    ...
    __out_ecount(cchBuffer) __opt LPSTR	lpszLongPath,
    __in DWORD    cchBuffer
    );
WINBASEAPI
DWORD
...
GetLongPathNameW(
    ...
    __out_ecount(cchBuffer) __opt LPWSTR	lpszLongPath,
    __in DWORD    cchBuffer
    );
#ifdef UNICODE
...
GetThreadStartInformation(
    IN HANDLE hThread,
    OUT LPVOID *lpStartAddress,
    OUT LPVOID *lpStartParameter
    );
...
HANDLE
WINAPI
CreateIoCompletionPortEx(
    IN HANDLE FileHandle,
    IN HANDLE ExistingCompletionPort,
    IN ULONG_PTR CompletionKey,
    IN DWORD NumberOfConcurrentThreads,
    IN UCHAR ObjectFlags
    );
...
ReadProcessMemory(
    IN HANDLE hProcess,
    IN LPCVOID lpBaseAddress,
    __out_bcount(nSize) LPVOID lpBuffer,
    __in SIZE_T nSize,
    __out __opt SIZE_T * lpNumberOfBytesRead
    );
...
WriteProcessMemory(
    ...
    __in SIZE_T nSize,
    __out __opt SIZE_T * lpNumberOfBytesWritten
    );
...
WriteFile(
    IN HANDLE hFile,
    __in_bcount(nNumberOfBytesToWrite) __opt LPCVOID lpBuffer,
    __in DWORD nNumberOfBytesToWrite,
    __out __opt LPDWORD lpNumberOfBytesWritten,
    IN LPOVERLAPPED lpOverlapped
    );
...
ReadFile(
    ...
    __out __opt LPDWORD lpNumberOfBytesRead,
    IN LPOVERLAPPED lpOverlapped
    );
...
SetEndOfFileEx(
    HANDLE hFile,
    LARGE_INTEGER lpNewEndOfFile
    );
    
typedef 
BOOLEAN (WINAPI *WALK_TREE_CALLBACK) ( 
        LPCWSTR lpFileName, 
        LPVOID Context 
        );
        
#define WALK_TREE_TRAVERSE_LINKS 1 
                                        
BOOLEAN                                            
WINAPI
WalkTreeW (
    IN LPCWSTR lpTreeRoot,
    IN LPCWSTR lpSearchPrefix OPTIONAL,
    IN ULONG dwFlags,
    IN WALK_TREE_CALLBACK Callback,
    IN PVOID Context
    );

#define DELETE_TREE_TRAVERSE_LINKS WALK_TREE_TRAVERSE_LINKS
...
#define SYMBOLIC_LINK_FLAG_RELATIVE_LINK        (0x2)
...
WINAPI
FlsAlloc(
    IN OPTIONAL PFLS_CALLBACK_FUNCTION lpCallback
    );
...
BackupRead(
    IN HANDLE hFile,
    __out_bcount(nNumberOfBytesToRead) __opt LPBYTE lpBuffer,
    __in DWORD nNumberOfBytesToRead,
    __out __opt LPDWORD lpNumberOfBytesRead,
    IN BOOL bAbort,
...
WINAPI
CreateReserveA(
    IN LPSECURITY_ATTRIBUTES lpReserveAttributes,
    IN DWORD dwDesiredAccess,
    IN LPCSTR lpName,
    IN OUT PDWORD pdwPeriod,
    IN OUT PDWORD pdwBudget,
    IN OUT PDWORD_PTR pdwpAffinityMask,
    IN DWORD fObjectFlags
    );
WINBASEAPI
HANDLE
...
#ifdef UNICODE
#define CreateReserve  CreateReserveW
#else
#define CreateReserve  CreateReserveA
#endif // !UNICODE
...
#ifdef UNICODE
#define OpenReserve  OpenReserveW
#else
#define OpenReserve  OpenReserveA
#endif // !UNICODE
...
BOOL
WINAPI
DisassociateProcessFromReserve (
    IN HANDLE hProcess
    );
...
BOOL
WINAPI
QueryReserveInformation (
    IN HANDLE hReserve,
    OUT PRESERVE_BASIC_INFORMATION pInformation
    );

WINBASEAPI
BOOL
WINAPI
QuerySystemReserveInformation (
    OUT PDWORD pdwMaximumBudget,
    OUT PDWORD pdwMinimumPeriod,
    OUT PDWORD pdwTimeGranularity
    );
...
GetEnvironmentVariableA(
    __in LPCSTR lpName,
    __out_ecount(nSize) __opt LPSTR lpBuffer,
    __in DWORD nSize
    );
WINBASEAPI
DWORD
...
GetEnvironmentVariableW(
    __in LPCWSTR lpName,
    __out_ecount(nSize) __opt LPWSTR lpBuffer,
    __in DWORD nSize
    );
#ifdef UNICODE
...
ExpandEnvironmentStringsA(
    __in LPCSTR lpSrc,
    __out_ecount(nSize) __opt LPSTR lpDst,
    __in DWORD nSize
    );
WINBASEAPI
DWORD
...
ExpandEnvironmentStringsW(
    __in LPCWSTR lpSrc,
    __out_ecount(nSize) __opt LPWSTR lpDst,
    __in DWORD nSize
    );
#ifdef UNICODE
...
GetFirmwareEnvironmentVariableA(
    __in LPCSTR lpName,
    __in LPCSTR lpGuid,
    __out_bcount(nSize) PVOID   pBuffer,
    __in DWORD    nSize
    );
WINBASEAPI
DWORD
...
SetFirmwareEnvironmentVariableA(
    __in LPCSTR lpName,
    __in LPCSTR lpGuid,
    __in_bcount(nSize) PVOID    pValue,
    __in DWORD    nSize
    );
WINBASEAPI
BOOL
...
#define  RESOURCE_ENUM_CODEBIN           0x0001
...
#define RESOURCE_UPDATE_CODEBIN   0x00000001
#define RESOURCE_UPDATE_MUIBIN      0x00000002

WINBASEAPI
HANDLE
WINAPI
BeginUpdateResourceExA(
    IN LPCSTR pFileName,
    IN BOOL bDeleteExistingResources,
    IN DWORD     dwFlags,
    IN LANGID     LangId
...
#ifdef UNICODE
#define BeginUpdateResourceEx  BeginUpdateResourceExW
#else
#define BeginUpdateResourceEx  BeginUpdateResourceExA
#endif // !UNICODE
...
#ifdef UNICODE
#define UpdateResourceEx  UpdateResourceExW
#else
#define UpdateResourceEx  UpdateResourceExA
#endif // !UNICODE

WINBASEAPI
BOOL
WINAPI
EndUpdateResourceExA(
    IN HANDLE      hUpdate,
    IN BOOL        fDiscard,
    IN DWORD    fFlags
    );
...
BOOL
WINAPI
EndUpdateResourceExW(
    IN HANDLE      hUpdate,
    IN BOOL        fDiscard,
    IN DWORD    fFlags
    );
#ifdef UNICODE
#define EndUpdateResourceEx  EndUpdateResourceExW
#else
#define EndUpdateResourceEx  EndUpdateResourceExA
#endif // !UNICODE
...
GlobalGetAtomNameA(
    __in ATOM nAtom,
    __out_ecount(nSize) LPSTR lpBuffer,
    __in int nSize
    );
WINBASEAPI
UINT
...
GetProfileStringA(
    ...
    IN LPCSTR lpKeyName,
    IN LPCSTR lpDefault,
    __out_ecount(nSize) LPSTR lpReturnedString,
    __in DWORD nSize
    );
WINBASEAPI
DWORD
...
GetProfileStringW(
    ...
    __out_ecount(nSize) LPWSTR lpReturnedString,
    __in DWORD nSize
    );
#ifdef UNICODE
...
GetPrivateProfileSectionNamesA(
    __out_ecount(nSize) LPSTR lpszReturnBuffer,
    __in DWORD nSize,
    IN LPCSTR lpFileName
    );
WINBASEAPI
...
GetPrivateProfileSectionNamesW(
    __out_ecount(nSize) LPWSTR lpszReturnBuffer,
    __in DWORD nSize,
    IN LPCWSTR lpFileName
    );
#ifdef UNICODE
...
GetPrivateProfileStructA(
    ...
    IN LPCSTR lpszKey,
    __out_bcount(uSizeStruct) LPVOID   lpStruct,
...
WritePrivateProfileStructA(
    IN LPCSTR lpszSection,
    IN LPCSTR lpszKey,
    __in_bcount(uSizeStruct) LPVOID   lpStruct,
    __in UINT     uSizeStruct,
    IN LPCSTR szFile
    );
WINBASEAPI
...
WINAPI
GetSystemDirectoryA(
    __out_ecount(uSize) __opt LPSTR lpBuffer,
    __in UINT uSize
    );
WINBASEAPI
UINT
WINAPI
GetSystemDirectoryW(
    __out_ecount(uSize) __opt LPWSTR lpBuffer,
    __in UINT uSize
    );
#ifdef UNICODE
...
GetDllDirectoryA(
    __in DWORD nBufferLength,
    __out_ecount(nBufferLength) LPSTR lpBuffer
    );
WINBASEAPI
DWORD
WINAPI
GetDllDirectoryW(
    __in DWORD nBufferLength,
    __out_ecount(nBufferLength) LPWSTR lpBuffer
    );
#ifdef UNICODE
...
QueryDosDeviceA(
    IN LPCSTR lpDeviceName,
    __out_ecount(ucchMax) LPSTR lpTargetPath,
    __in DWORD ucchMax
    );
WINBASEAPI
DWORD
...
QueryDosDeviceW(
    IN LPCWSTR lpDeviceName,
    __out_ecount(ucchMax) LPWSTR lpTargetPath,
    __in DWORD ucchMax
    );
#ifdef UNICODE
...
WINAPI
LookupAccountSidA(
    __in __opt LPCSTR lpSystemName,
    __in PSID Sid,
    __out_ecount(*cchName) __opt LPSTR Name,
    __inout LPDWORD cchName,
    __out_ecount(*cchReferencedDomainName) __opt LPSTR ReferencedDomainName,
...
WINAPI
LookupAccountSidW(
    __in __opt LPCWSTR lpSystemName,
    __in PSID Sid,
    __out_ecount(*cchName) __opt LPWSTR Name,
    __inout LPDWORD cchName,
    __out_ecount(*cchReferencedDomainName) __opt LPWSTR ReferencedDomainName,
    __inout LPDWORD cchReferencedDomainName,
    __out PSID_NAME_USE peUse
    );
#ifdef UNICODE
...
LookupAccountNameA(
    ...
    __out_bcount(*cbSid) __opt PSID Sid,
...
LookupPrivilegeDisplayNameA(
    __in __opt LPCSTR lpSystemName,
    __in LPCSTR lpName,
    __out_ecount(*cchDisplayName) __opt LPSTR lpDisplayName,
...
LookupPrivilegeDisplayNameW(
    __in __opt LPCWSTR lpSystemName,
    __in LPCWSTR lpName,
    __out_ecount(*cchDisplayName) __opt LPWSTR lpDisplayName,
    __inout LPDWORD cchDisplayName,
    __out LPDWORD lpLanguageId
    );
#ifdef UNICODE
...
typedef VOID (NTAPI *PTP_WIN32_IO_CALLBACK)(
    OUT PTP_CALLBACK_INSTANCE Instance,
    IN  PVOID                 Context,
    IN  PVOID                 Overlapped,
    IN  ULONG                 IoResult,
...
FORCEINLINE
VOID
SetThreadpoolCallbackGang(
    IN OUT PTP_CALLBACK_ENVIRON     pcbe,
    IN     PTP_GANG                 ptpg,
    IN     PTP_GANG_CANCEL_CALLBACK pfng
    )
{
    TpSetCallbackGang(pcbe, ptpg, pfng);
}
...
FORCEINLINE
VOID
DestroyThreadpoolCallbackEnvironment(
    IN PTP_CALLBACK_ENVIRON pcbe
    )
{
...
SubmitThreadpoolCallback(
    ...
    IN PVOID                pv,
    IN PTP_CALLBACK_ENVIRON pcbe OPTIONAL
    );
...
SetThreadpoolWait(
    IN PTP_WAIT  pwa,
    IN HANDLE    h,
    IN PFILETIME pftTimeout OPTIONAL
    );
...
BOOL
WINAPI
DonateCurrentThreadToThreadpool(
    IN PTP_POOL            ptpp OPTIONAL,
    IN PTP_SIMPLE_CALLBACK pfns OPTIONAL,
    IN PVOID               pv
    );
...
WINAPI
GetOSProductNameA(
    OUT LPSTR pszProductName,
    IN UINT cchProductName,
    IN DWORD dwFlags
    );
WINBASEAPI
BOOL
WINAPI
GetOSProductNameW(
    OUT LPWSTR pszProductName,
    IN UINT cchProductName,
    IN DWORD dwFlags
    );
#ifdef UNICODE
#define GetOSProductName  GetOSProductNameW
#else
#define GetOSProductName  GetOSProductNameA
#endif // !UNICODE
...
MapUserPhysicalPages(
    ...
    IN OPTIONAL PULONG_PTR PageArray
    );
...
#if (_WIN32_WINNT >= 0x0501) || (_WIN32_FUSION >= 0x0100)
...
#if (_WIN32_WINNT >= 0x0501) || (_WIN32_FUSION >= 0x0100) || ISOLATION_AWARE_ENABLED
...
#endif // (_WIN32_WINNT > 0x0501) || (_WIN32_FUSION >= 0x0100)
