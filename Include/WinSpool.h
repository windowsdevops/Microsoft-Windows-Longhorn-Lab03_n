...
#define PRINTER_ATTRIBUTE_ALLOW_EMF         0x00010000 //Used for CSR Guid Printers, if they use server-side rendering.
...
#define PRINTER_ATTRIBUTE_DETACHED_COMMENT  0x00100000
#define PRINTER_ATTRIBUTE_EDOC              0x00200000
...
typedef struct _JOB_EXTRAA {
    LPSTR     pOriginalDatatype;
    PDEVMODEA pDevMode;
} JOB_EXTRAA, *PJOB_EXTRAA;
typedef struct _JOB_EXTRAW {
    LPWSTR    pOriginalDatatype;
    PDEVMODEW pDevMode;
} JOB_EXTRAW, *PJOB_EXTRAW;
#ifdef UNICODE
typedef JOB_EXTRAW JOB_EXTRA;
typedef PJOB_EXTRAW PJOB_EXTRA;
#else
typedef JOB_EXTRAA JOB_EXTRA;
typedef PJOB_EXTRAA PJOB_EXTRA;
#endif // UNICODE
...
#define JOB_STATUS_FAKING_DATATYPE      0x00004000
...
typedef struct _FORM_INFO_2A {
    ...
    RECTL           ImageableArea;
    LPCSTR            pDefaultName;
    STRING_RESOURCE_TYPE   StringType;
    union
    {
        struct
        {
...
#ifdef UNICODE
#define EnumFormsEx  EnumFormsExW
#else
#define EnumFormsEx  EnumFormsExA
#endif // !UNICODE
...
typedef enum _PRINTER_OPTION_FLAGS
{
    ...
    PRINTER_OPTION_XCV            = 1 << 2,
    PRINTER_OPTION_CLIENT_CHANGE  = 1 << 3,
...
OpenPrinter2A(
   ...
   OUT  LPHANDLE                phPrinter,
   IN   PPRINTER_DEFAULTSA      pDefault,
   IN   PPRINTER_OPTIONSA       pOptions
   );
BOOL
WINAPI
OpenPrinter2W(
   ...
   OUT  LPHANDLE                phPrinter,
   IN   PPRINTER_DEFAULTSW      pDefault,
   IN   PPRINTER_OPTIONSW       pOptions
   );
#ifdef UNICODE
...
#define PRINTER_CONNECTION_DPS                  0x00000008      // printer is dps connection
#define PRINTER_CONNECTION_ONLY                 0x00000010      // printer added as connection only
#define PRINTER_CONNECTION_MISMATCH             0x00000020      // printer should be mismatched

typedef enum _PRINTER_CONNECTION_RESULT_FLAGS
{
    PRINTER_CONNECTION_RESULT_RPC  = 1 << 0,    // printer was found via rpc
} PRINTER_CONNECTION_RESULT_FLAGS;
...
BOOL
WINAPI
DeletePrinterConnection2A(
   IN    LPCSTR       pszName,
   IN    DWORD          dwReserved
   );
BOOL
WINAPI
DeletePrinterConnection2W(
   IN    LPCWSTR       pszName,
   IN    DWORD          dwReserved
   );
#ifdef UNICODE
#define DeletePrinterConnection2  DeletePrinterConnection2W
#else
#define DeletePrinterConnection2  DeletePrinterConnection2A
#endif // !UNICODE
...
AddJobEx(
    ...
    PrintPropertiesCollection*      pCollection,
    PrintPropertiesCollection**     ppJobDataOutCollection
    );
...
GetJobEx(
    HANDLE                         hPrinter,
    ULONG                          jobIdentifier,
    PrintPropertiesCollection**    ppJobOutCollection
    );  

HRESULT
FreePropertyCollection(
    PrintPropertiesCollection*     pCollection
    );
...
ReportJobProcessingProgress(
    HANDLE                         hPrinter,
    ULONG                          jobIdentifier,
    EPrintJobOperation             jobOperation,
    EPrintJobProgress              jobProgress,			
    PrintPropertiesCollection*     pCollection
    );
