...
SHSTDAPI_(UINT) DragQueryFileA(HDROP hDrop, UINT iFile, __out_ecount(cch) __opt LPSTR lpszFile, UINT cch);
SHSTDAPI_(UINT) DragQueryFileW(HDROP hDrop, UINT iFile, __out_ecount(cch) __opt LPWSTR lpszFile, UINT cch);
#ifdef UNICODE
...
SHSTDAPI_(HICON) ExtractAssociatedIconA(HINSTANCE hInst, __in LPSTR lpIconPath, __in LPWORD lpiIcon);
SHSTDAPI_(HICON) ExtractAssociatedIconW(HINSTANCE hInst, __in LPWSTR lpIconPath, __in LPWORD lpiIcon);
#ifdef UNICODE
...
SHSTDAPI_(UINT) ExtractIconExA(LPCSTR lpszFile, int nIconIndex, __out __opt HICON *phiconLarge, __out __opt HICON *phiconSmall, UINT nIcons);
SHSTDAPI_(UINT) ExtractIconExW(LPCWSTR lpszFile, int nIconIndex, __out __opt HICON *phiconLarge, __out __opt HICON *phiconSmall, UINT nIcons);
#ifdef UNICODE
...
#define FO_LINK                    0x0005
#define FO_APPLYPROPERTIES         0x0006
...
#define SEE_MASK_FLAG_NOASYNC      0x00000100
#define SEE_MASK_FLAG_DDEWAIT      SEE_MASK_FLAG_NOASYNC      
...
SHSTDAPI_(DWORD_PTR) SHGetFileInfoA(LPCSTR pszPath, DWORD dwFileAttributes, __out_bcount(cbFileInfo) __opt SHFILEINFOA *psfi,
    UINT cbFileInfo, UINT uFlags);
SHSTDAPI_(DWORD_PTR) SHGetFileInfoW(LPCWSTR pszPath, DWORD dwFileAttributes, __out_bcount(cbFileInfo) __opt SHFILEINFOW *psfi,
    UINT cbFileInfo, UINT uFlags);
#ifdef UNICODE
...
SHSTDAPI_(BOOL) SHGetDiskFreeSpaceExW(LPCWSTR pszDirectoryName, ULARGE_INTEGER* pulFreeBytesAvailableToCaller,
    ULARGE_INTEGER* pulTotalNumberOfBytes, ULARGE_INTEGER* pulTotalNumberOfFreeBytes);
#ifdef UNICODE
...
// ShellMessageBoxA/W are now exported from shlwapi, define accordingly
#if !defined(_SHLWAPI_)
#define MOVEDTOSHLWAPIAPI   DECLSPEC_IMPORT
#else
#define MOVEDTOSHLWAPIAPI
#endif

EXTERN_C MOVEDTOSHLWAPIAPI int _cdecl ShellMessageBoxA(
    HINSTANCE hAppInst,
    HWND hWnd,
    LPCSTR lpcText,
    LPCSTR lpcTitle,
...
EXTERN_C MOVEDTOSHLWAPIAPI int _cdecl ShellMessageBoxW(
    HINSTANCE hAppInst,
    HWND hWnd,
    LPCWSTR lpcText,
    LPCWSTR lpcTitle,
