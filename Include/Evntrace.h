...
#define APPL_EVENT_LOGGER_NAMEW     L"Applications Event Logger"
#define SECURITY_EVENT_LOGGER_NAMEW L"Security Event Logger"
#define SYSTEM_EVENT_LOGGER_NAMEW   L"System Event Logger"
#define CONTEXT_LOGGER_NAMEW        SYSTEM_EVENT_LOGGER_NAMEW   
...
#define APPL_EVENT_LOGGER_NAMEA     "Applications Event Logger"
#define SECURITY_EVENT_LOGGER_NAMEA "Security Event Logger"
#define SYSTEM_EVENT_LOGGER_NAMEA   "System Event Logger"
#define CONTEXT_LOGGER_NAMEA        SYSTEM_EVENT_LOGGER_NAMEA
...
//
// Message types for Event Log traces
//
#define EVENT_LOG_MESSAGE_NUMBER_V1				0x0A     //Regular event log event message type
...
#define EVENT_TRACE_NONSTOPPABLE_MODE       0x00000040  // Session cannot be stopped
                                                        // Autologger only
...
#define TRACE_HEADER_FLAG_PERSIST_EVENT     0x00000400
...
#define TRACE_HEADER_FLAG_NO_HEADER         0x00200000 // Trace without header
#define TRACE_HEADER_FLAG_USE_EVENTID       0x00400000 // Trace with correlation id
#define TRACE_HEADER_FLAG_USE_TYPE_ENCODING 0x00800000 // Trace with type encoded user data
...
typedef struct _EVENT_TRACE_HEADER {        // overlays WNODE_HEADER
    ...
    union {
        ...
        GUID        CorrelationId;          // Correlation Id
    };
    union {
...
typedef struct _ETW_ENCODED_TRACE_DATA_ITEM {
    USHORT DataType;
    USHORT Length;
} ETW_ENCODED_TRACE_DATA_ITEM, *PETW_ENCODED_TRACE_DATA_ITEM;

typedef struct _ETW_TYPE_ENCODED_TRACE_DATA {
    ULONG NumItems;
    ETW_ENCODED_TRACE_DATA_ITEM TraceDataItem[1];
} ETW_TYPE_ENCODED_TRACE_DATA, *PETW_TYPE_ENCODED_TRACE_DATA;
...
//
// Context Event Structures
//


#define GETLEVELFROMEVENTIDD(EventIDD) EventIDD.Level


//
// The TRACE_CONTEXT_HEADER gives information about the contexts in the rest of the message
// N.B.: Must be eight-byte-aligned
//
typedef struct _CONTEXT_OFFSET {
    USHORT          Flags ;
    USHORT          Type ;
    USHORT          Size ;
    USHORT          Offset ;
} CONTEXT_OFFSET, *PCONTEXT_OFFSET ;

// N.B.: must be eight-byte-aligned
typedef  struct  _WDI_CONTEXT_HEADER {
            USHORT          ContextVersion ;
            USHORT          NumContexts ;   
            ULONG           Reserved;           // for align
            CONTEXT_OFFSET  ContextOffset[ANYSIZE_ARRAY] ;
} WDI_CONTEXT_HEADER, *PWDI_CONTEXT_HEADER ;


//
// TRACE_CONTEXT_EVENT_INTERNAL is the structure as it is used internally
//
typedef struct _TRACE_CONTEXT_EVENT_INTERNAL {
    // The following fields overlay onto public structure TRACE_CONTEXT_EVENT.
    PEVENT_TRACE_HEADER Header ;
    PVOID               UserData ;

    // Private from here on out. Be sure to maintain alignment.
    ULONG               SubComponent ;
    UCHAR               Level ;

    UCHAR               Reserved1;
    USHORT              Reserved2;               // for align

    EVENT_TRACE_HEADER  EventTraceHeader ;
    WDI_CONTEXT_HEADER  ContextHeader ;
} TRACE_CONTEXT_EVENT_INTERNAL, *PTRACE_CONTEXT_EVENT_INTERNAL ;

typedef struct _TRACEPTR {
	ULONG		Type ;
	ULONG		Size ;
	ULONG_PTR	PData ;
} TRACEPTR, *PTRACEPTR ;
...
typedef struct _TRACE_ENABLE_CONTEXT_BLOCK {
    TRACEHANDLE SessionHandle;      // Actual Id of the logger
    ULONG EnableFlags;              // Enable flags passed by control caller
    USHORT Level;                   // Enable level passed by control caller
    USHORT Reserved;                // Reserved
    BOOLEAN IsEnabled;              // Enabled or Disabled
} TRACE_ENABLE_CONTEXT_BLOCK, *PTRACE_ENABLE_CONTEXT_BLOCK;
...
// Used by SetEnableInstrumentationNotification() and EnableTraceEx()
typedef struct  _TRACE_ENABLE_FILTER_ITEM_INFO {
    ULONG FilterType;               // Filter type
    ULONG DataSize;                 // Filter data size
    union 
    {
        PVOID FilterDataPtr;        // Pointer to the filter data
        ULONG64 FilterDataPtr64;    // For allignment
    };
} TRACE_ENABLE_FILTER_ITEM_INFO, *PTRACE_ENABLE_FILTER_ITEM_INFO;
...
typedef struct _EVENT_TRACE {
    ...
    union {
        ...
        ULONG               Flags;              // Event Header Flag
    };
    union {
        GUID                ParentGuid;         // Parent Guid;
        GUID                CorrelationId;      // Parent Guid;
    };
...
EnableTraceEx(
    IN LPCGUID ControlGuid,
    IN ULONG TargetPID,
    IN PTRACE_ENABLE_CONTEXT_BLOCK EnableContext,
    IN PTRACE_ENABLE_FILTER_ITEM_INFO EnableFilterItem
    );
...
EXTERN_C
PTRACE_ENABLE_FILTER_ITEM_INFO
WMIAPI
GetTraceEnableFilter(
    IN PVOID Buffer
    );
