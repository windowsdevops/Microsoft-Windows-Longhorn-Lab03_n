...
} PEER_CONNECTION_INFO, *PPEER_CONNECTION_INFO;
...
HRESULT WINAPI PeerGraphGetConnectionInfo(
                IN  HGRAPH       hGraph,
                IN  ULONGLONG    ullConnectionId,
                OUT PPEER_CONNECTION_INFO * ppConnectionInfo);
...
typedef enum peer_group_event_type_tag {
    ...
    PEER_GROUP_EVENT_NODE_CHANGED               = 7,
...
typedef struct peer_identity_info_tag
{
    DWORD                   dwSize;
    DWORD                   dwFlags;
    PWSTR                   pwzPeerName;
    PWSTR                   pwzFriendlyName;
    CERT_PUBLIC_KEY_INFO*   pPublicKey;
} PEER_IDENTITY_INFO, *PPEER_IDENTITY_INFO;
...
HRESULT WINAPI PeerGroupGetConnectionInfo(
                IN  HGROUP       hGroup,
                IN  ULONGLONG    ullConnectionId,
                OUT PPEER_CONNECTION_INFO * ppConnectionInfo);
...
HRESULT WINAPI PeerIdentityParseXML(
                IN  PCWSTR       pwzIdentityXML,
                OUT PPEER_IDENTITY_INFO* ppIdentityInfo);

HRESULT WINAPI PeerIdentityParseExportXML(
                IN  PCWSTR       pwzExportXML,
                OUT PWSTR*       ppwzIdentityName);
...
// API Definitions
HRESULT WINAPI PeerPnrpToDnsName(
                IN  PCWSTR      pwszPnrpNameStr,
                OUT PWSTR       pwszDnsNameStr,
                IN OUT ULONG *  pcchDnsName);

HRESULT WINAPI PeerDnsToPnrpName(
                IN  PCWSTR      pwszDnsNameStr,
                OUT PWSTR       pwszPnrpNameStr,
                IN OUT ULONG *  pcchPnrpName);
...
#define PEER_E_INVALID_XML                   MAKE_HRESULT(SEVERITY_ERROR, FACILITY_P2P, 0x0704)
...
#define PEER_E_INVALID_IDENTITY_INFO         MAKE_HRESULT(SEVERITY_ERROR, FACILITY_P2P, 0x2084)
...
// Pnrp helpers errors
#define PEER_E_CANNOT_CONVERT_PNRP_NAME      MAKE_HRESULT(SEVERITY_ERROR, FACILITY_P2P, 0x4001)
#define PEER_E_INVALID_PEER_DNS_NAME         MAKE_HRESULT(SEVERITY_ERROR, FACILITY_P2P, 0x4002)
...
#define PEER_E_INSUFFICIENT_BUFFER           MAKE_HRESULT(SEVERITY_ERROR, FACILITY_WIN32, ERROR_INSUFFICIENT_BUFFER)
