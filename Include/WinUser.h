...
#define HSHELL_MINMAXRESTORE        15
#define HSHELL_POSCHANGED           16
...
ToUnicodeEx(
    __in UINT wVirtKey,
    __in UINT wScanCode,
    __in_bcount(256) __opt CONST BYTE *lpKeyState,
    __out_ecount(cchBuff) LPWSTR pwszBuff,
    __in int cchBuff,
    __in UINT wFlags,
    __in __opt HKL dwhkl);
#endif /* WINVER >= 0x0400 */
...
GetKeyboardLayoutList(
        __in int nBuff,
        __out_ecount(nBuff) __opt HKL FAR *lpList);
...
CreateDesktopA(
    ...
    __in __opt LPSECURITY_ATTRIBUTES lpsa);
WINUSERAPI
HDESK
WINAPI
CreateDesktopW(
...
WINAPI
EnumDesktopsA(
    __in __opt HWINSTA hwinsta,
    __in DESKTOPENUMPROCA lpEnumFunc,
    __in __opt LPARAM lParam);
WINUSERAPI
BOOL
WINAPI
EnumDesktopsW(
...
#define CWF_VALID_FLAGS          0x0001L
...
GetUserObjectSecurity(
    __in HANDLE hObj,
    __in PSECURITY_INFORMATION pSIRequested,
    __out_bcount(nLength) __opt PSECURITY_DESCRIPTOR pSID,
    __in DWORD nLength,
...
GetUserObjectInformationA(
    __in HANDLE hObj,
    __in int nIndex,
    __out_bcount(nLength) __opt PVOID pvInfo,
    __in DWORD nLength,
    __out __opt LPDWORD lpnLengthNeeded);
WINUSERAPI
BOOL
WINAPI
...
typedef struct tagWNDCLASSEXA {
    ...
    __in LPCSTR      lpszMenuName;
    __in LPCSTR      lpszClassName;
    /* Win 4.0 */
    HICON       hIconSm;
...
typedef struct tagWNDCLASSEXW {
    ...
    HCURSOR     hCursor;
    HBRUSH      hbrBackground;
    __in LPCWSTR     lpszMenuName;
    __in LPCWSTR     lpszClassName;
    /* Win 4.0 */
    HICON       hIconSm;
...
#if(WINVER >= 0x0500)
#define WM_UILANGUAGEADD                0x0260
#define WM_UILANGUAGEREMOVE             0x0261
#endif /* WINVER >= 0x0500 */
...
SendMessageTimeoutA(
    ...
    __in UINT fuFlags,
    __in UINT uTimeout,
    __out __opt PDWORD_PTR lpdwResult);
WINUSERAPI
LRESULT
WINAPI
SendMessageTimeoutW(
...
BroadcastSystemMessageExA(
    __in DWORD flags,
    __in __opt LPDWORD lpInfo,
    __in UINT Msg,
    __in __opt WPARAM wParam,
    __in __opt LPARAM lParam,
    __out __opt PBSMINFO pbsmInfo);
WINUSERAPI
long
WINAPI
...
RegisterPowerSettingNotification(
    __in HANDLE hRecipient,
    __in CONST LPGUID PowerSettingGuid,
    __in DWORD Flags
    );
...
UnregisterClassA(
    ...
    __in __opt HINSTANCE hInstance);
WINUSERAPI
BOOL
WINAPI
UnregisterClassW(
...
CreateWindowExA(
    ...
    __in __opt LPCSTR lpClassName,
    __in __opt LPCSTR lpWindowName,
    __in DWORD dwStyle,
    __in int X,
    __in int Y,
    __in int nWidth,
    __in int nHeight,
    __in __opt HWND hWndParent,
    __in __opt HMENU hMenu,
    __in __opt HINSTANCE hInstance,
    __in __opt LPVOID lpParam);
WINUSERAPI
HWND
WINAPI
CreateWindowExW(
    __in DWORD dwExStyle,
    __in __opt LPCWSTR lpClassName,
    __in __opt LPCWSTR lpWindowName,
    __in DWORD dwStyle,
    __in int X,
    __in int Y,
    __in int nWidth,
...
UpdateLayeredWindow(
    __in HWND hWnd,
    __in __opt HDC hdcDst,
    __in __opt POINT *pptDst,
    __in __opt SIZE *psize,
    __in __opt HDC hdcSrc,
    __in __opt POINT *pptSrc,
    __in COLORREF crKey,
    __in __opt BLENDFUNCTION *pblend,
    __in DWORD dwFlags);
#endif
...
GetLayeredWindowAttributes(
    __in HWND hwnd,
    __out __opt COLORREF *pcrKey,
    __out __opt BYTE *pbAlpha,
    __out __opt DWORD *pdwFlags);
...
SetWindowPos(
    __in HWND hWnd,
    __in __opt HWND hWndInsertAfter,
    __in int X,
    __in int Y,
    __in int cx,
    __in int cy,
...
CreateDialogParamA(
    ...
    __in __opt HWND hWndParent,
    __in __opt DLGPROC lpDialogFunc,
    __in __opt LPARAM dwInitParam);
WINUSERAPI
HWND
WINAPI
CreateDialogParamW(
...
GetPriorityClipboardFormat(
    __out_ecount(cFormats) UINT *paFormatPriorityList,
    __in int cFormats);
...
GetKeyNameTextA(
    __in LONG lParam,
    __out_ecount(nSize) LPSTR lpString,
    __in int nSize
    );
WINUSERAPI
int
...
GetKeyNameTextW(
    __in LONG lParam,
    __out_ecount(nSize) LPWSTR lpString,
    __in int nSize
    );
#ifdef UNICODE
...
ToAscii(
    __in UINT uVirtKey,
    __in UINT uScanCode,
    __in_ecount(256) __opt  CONST BYTE *lpKeyState,
    __out LPWORD lpChar,
    __in UINT uFlags);
...
#endif /* _WIN32_WINNT >= 0x0500 */
#if(_WIN32_WINNT >= 0x0501)
#define KEYEVENTF_PRESSUREDATA   0x0010
#define KEYEVENTF_PRESSUREUPDATE 0x0030    // Update must include data
#endif /* _WIN32_WINNT >= 0x0501 */
...
keybd_event(
    ...
    __in  BYTE bScan,
    __in  DWORD dwFlags,
    __in  __opt ULONG_PTR dwExtraInfo);
...
#define MOUSEEVENTF_WHEEL       0x00800 /* wheel button rolled */
#if (_WIN32_WINNT >= 0x0600)
#define MOUSEEVENTF_HWHEEL      0x01000 /* hwheel button rolled */
#endif
#define MOUSEEVENTF_VIRTUALDESK 0x04000 /* map to entire virtual desktop */
#define MOUSEEVENTF_ABSOLUTE    0x08000 /* absolute move */
#define MOUSEEVENTF_ACTUAL      0x10000 /* don't touch the coords at all */
...
typedef struct tagKEYBDINPUT {
    ...
    ULONG_PTR dwExtraInfo;
#if(_WIN32_WINNT >= 0x0501)
    BYTE    PressureData;      // ignored if KEYEVENTF_PRESSUREDATA not specified
#endif /* _WIN32_WINNT >= 0x0501 */
...
WINAPI
SendInputEx(
    __in HWND    hwnd,        // hwnd this input should go to
    __in UINT    cInputs,     // number of input in the array
    __in_ecount(cInputs) LPINPUT pInputs,     // array of inputs
    __in int     cbSize);     // sizeof(INPUT)
#endif /* _WIN32_WINNT >= 0x0501 */
...
MsgWaitForMultipleObjects(
    __in DWORD nCount,
    __in_ecount(nCount) __opt CONST HANDLE *pHandles,
    __in BOOL fWaitAll,
    __in DWORD dwMilliseconds,
...
SetTimer(
    ...
    __in UINT uElapse,
    __in __opt TIMERPROC lpTimerFunc);
...
CreateAcceleratorTableA(
    __in_ecount(cEntries) LPACCEL, __in int cEntries);
WINUSERAPI
HACCEL
WINAPI
...
CopyAcceleratorTableA(
    ...
    __out_ecount(cAccelEntries) __opt LPACCEL lpAccelDst,
...
#if(_WIN32_WINNT >= 0x0501)
#define SM_WAITTOKILLTIMEOUT 89
#define SM_WAITTOKILLSERVICESTIMEOUT 90
#define SM_HUNGAPPTIMEOUT 91

BOOL IsServicesProcess(
    __in DWORD dwProcessId);
#endif /* _WIN32_WINNT >= 0x0501 */

#if(_WIN32_WINNT >= 0x0501)
#define SM_EDUCATION            92
#endif /* _WIN32_WINNT >= 0x0501 */

#if(_WIN32_WINNT >= 0x0501)
#define SM_PROCESSTERMINATETIMEOUT 93
#endif /* _WIN32_WINNT >= 0x0501 */

#if(_WIN32_WINNT >= 0x0600)
#define SM_MOUSEHORIZONTALWHEELPRESENT    94
#endif /* _WIN32_WINNT >= 0x0600 */
...
#elif WINVER == 0x501
#define SM_CMETRICS             94
#else
#define SM_CMETRICS             95
#endif
...
ChangeMenuW(
    __in HMENU hMenu,
    __in UINT cmd,
    __in __opt LPCWSTR lpszNewItem,
    __in UINT cmdInsert,
    __in UINT flags);
#ifdef UNICODE
...
GetMenuStringA(
    __in HMENU hMenu,
    __in UINT uIDItem,
    __out_ecount(nMaxCount) __opt LPSTR lpString,
    __in int nMaxCount,
    __in UINT uFlag);
WINUSERAPI
int
...
GetMenuStringW(
    __in HMENU hMenu,
    __in UINT uIDItem,
    __out_ecount(nMaxCount) __opt LPWSTR lpString,
    __in int nMaxCount,
    __in UINT uFlag);
#ifdef UNICODE
...
SetMenuItemBitmaps(
    ...
    __in UINT uPosition,
    __in UINT uFlags,
    __in __opt HBITMAP hBitmapUnchecked,
    __in __opt HBITMAP hBitmapChecked);
...
WINUSERAPI UINT    WINAPI GetMenuDefaultItem( __in HMENU hMenu, __in UINT fByPos, __in UINT gmdiFlags);
WINUSERAPI BOOL    WINAPI SetMenuDefaultItem( __in HMENU hMenu, __in UINT uItem,  __in UINT fByPos);

WINUSERAPI BOOL    WINAPI GetMenuItemRect( __in __opt HWND hWnd, __in HMENU hMenu, __in UINT uItem, __out LPRECT lprcItem);
WINUSERAPI int     WINAPI MenuItemFromPoint( __in __opt HWND hWnd, __in HMENU hMenu, __in POINT ptScreen);
#endif /* WINVER >= 0x0400 */
...
DragObject(
    ...
    __in HWND hwndFrom,
    __in UINT fmt,
    __in __opt ULONG_PTR data,
    __in __opt HCURSOR hcur);
...
DrawTextA(
    __in HDC hDC,
    __in_ecount(nCount) LPCSTR lpString,
    __in int nCount,
...
DrawTextW(
    __in HDC hDC,
    __in_ecount(nCount) LPCWSTR lpString,
    __in int nCount,
    __inout LPRECT lpRect,
    __in UINT uFormat);
#ifdef UNICODE
...
DrawTextExA(
    __in HDC hdc,
    __inout_ecount(nCount) __opt LPSTR lpString,
    __in int nCount,
    __inout LPRECT lpRect,
    __in UINT format,
    __in __opt LPDRAWTEXTPARAMS);
WINUSERAPI
int
WINAPI
DrawTextExW(
    __in HDC hdc,
    __inout_ecount(nCount) __opt LPWSTR lpString,
    __in int nCount,
    __inout LPRECT lpRect,
    __in UINT format,
    __in __opt LPDRAWTEXTPARAMS);
...
GrayStringA(
    __in HDC hDC,
    __in __opt HBRUSH hBrush,
    __in __opt GRAYSTRINGPROC lpOutputFunc,
    __in __opt LPARAM lpData,
    __in int nCount,
    __in int X,
    __in int Y,
    __in int nWidth,
...
DrawStateA(
    __in HDC hdc,
    __in __opt HBRUSH hbrFore,
    __in __opt DRAWSTATEPROC qfnCallBack,
    __in __opt LPARAM lData,
    __in __opt WPARAM wData,
    __in int x,
    __in int y,
    __in int cx,
    __in int cy,
...
TabbedTextOutA(
    ...
    __in_ecount(nTabPositions) __opt CONST INT *lpnTabStopPositions,
    __in int nTabOrigin);
WINUSERAPI
LONG
WINAPI
...
#define SWR_CLIPRGN        0x0000
#define SWR_NEWFRAMERGN    0x0001
#define SWR_REDRAW         0x0002
#define SWR_NONOTIFY       0x0004
...
RedrawWindow(
    __in __opt HWND hWnd,
    __in __opt CONST RECT *lprcUpdate,
    __in __opt HRGN hrgnUpdate,
    __in UINT flags);
...
ScrollWindow(
    ...
    __in int YAmount,
    __in __opt CONST RECT *lpRect,
    __in __opt CONST RECT *lpClipRect);
...
ScrollDC(
    ...
    __in int dx,
    __in int dy,
    __in __opt CONST RECT *lprcScroll,
    __in __opt CONST RECT *lprcClip,
    __in __opt HRGN hrgnUpdate,
    __out __opt LPRECT lprcUpdate);
...
ScrollWindowEx(
    ...
    __in int dx,
    __in int dy,
    __in __opt CONST RECT *prcScroll,
    __in __opt CONST RECT *prcClip,
    __in __opt HRGN hrgnUpdate,
    __out __opt LPRECT prcUpdate,
    __in UINT flags);
...
MessageBoxA(
    __in __opt HWND hWnd,
    __in __opt LPCSTR lpText,
    __in __opt LPCSTR lpCaption,
    __in UINT uType);
WINUSERAPI
int
WINAPI
MessageBoxW(
    __in __opt HWND hWnd,
    __in __opt LPCWSTR lpText,
    __in __opt LPCWSTR lpCaption,
    __in UINT uType);
#ifdef UNICODE
...
WINAPI
MapWindowPoints(
    __in __opt HWND hWndFrom,
    __in __opt HWND hWndTo,
...
WINUSERAPI HWND    WINAPI ChildWindowFromPointEx( __in HWND hwnd, __in POINT pt, __in UINT flags);
#endif /* WINVER >= 0x0400 */
...
FindWindowExA(
    __in __opt HWND hWndParent,
    __in __opt HWND hWndChildAfter,
    __in __opt LPCSTR lpszClass,
    __in __opt LPCSTR lpszWindow);
WINUSERAPI
HWND
WINAPI
FindWindowExW(
    __in __opt HWND hWndParent,
    __in __opt HWND hWndChildAfter,
    __in __opt LPCWSTR lpszClass,
    __in __opt LPCWSTR lpszWindow);
#ifdef UNICODE
...
GetClassNameA(
    __in HWND hWnd,
    __out_ecount(nMaxCount) LPSTR lpClassName,
    __in int nMaxCount);
WINUSERAPI
int
WINAPI
...
GetWindowThreadProcessId(
    __in HWND hWnd,
    __out __opt LPDWORD lpdwProcessId);
...
ULONG
WINAPI
GetWindowCloseTimestamp(
    __in HWND hwnd);

WINUSERAPI
ULONG
WINAPI
GetWindowHungTimestamp(
    __in HWND hwnd);
...
LoadBitmapA(
    __in __opt HINSTANCE hInstance,
    __in __opt LPCSTR lpBitmapName);
WINUSERAPI
HBITMAP
WINAPI
LoadBitmapW(
    __in __opt HINSTANCE hInstance,
    __in __opt LPCWSTR lpBitmapName);
#ifdef UNICODE
...
LoadCursorA(
    __in __opt HINSTANCE hInstance,
    __in __opt LPCSTR lpCursorName);
WINUSERAPI
HCURSOR
WINAPI
LoadCursorW(
    __in __opt HINSTANCE hInstance,
    __in __opt LPCWSTR lpCursorName);
#ifdef UNICODE
...
WINAPI
CreateCursor(
    __in __opt HINSTANCE hInst,
    __in int xHotSpot,
    __in int yHotSpot,
    __in int nWidth,
    __in int nHeight,
    __in __opt CONST VOID *pvANDPlane,
    __in __opt CONST VOID *pvXORPlane);
...
PrivateExtractIconsA(
    ...
    __in int      cxIcon,
    __in int      cyIcon,
    __out_ecount(nIcons) __opt  HICON   *phicon,
    __out __opt UINT    *piconid,
    __in UINT     nIcons,
    __in UINT     flags);
WINUSERAPI 
UINT 
...
CreateIcon(
    ...
    __in __opt CONST BYTE *lpbANDbits,
    __in __opt CONST BYTE *lpbXORbits);
...
WINUSERAPI BOOL WINAPI DrawIconEx(
    ...
    __in __opt HBRUSH hbrFlickerFreeDraw,
    __in UINT diFlags);
...
#if(WINVER >= 0x0501)
#ifdef RC_INVOKED
#define IDI_CONSOLE 32518
#else
#define IDI_CONSOLE MAKEINTRESOURCE(32518)
#endif
#endif /* WINVER >= 0x0501 */
...
LoadStringA(
    __in __opt HINSTANCE hInstance,
    __in UINT uID,
    __out_ecount(nBufferMax) __opt LPSTR lpBuffer,
    __in int nBufferMax);
WINUSERAPI
int
WINAPI
LoadStringW(
    __in __opt HINSTANCE hInstance,
    __in UINT uID,
    __out_ecount(nBufferMax) __opt LPWSTR lpBuffer,
    __in int nBufferMax);
#ifdef UNICODE
...
#define BS_STYLEMASK        0x0000FF2FL
...
DlgDirSelectExA(
    __in HWND hDlg,
    __out_ecount(nCount) LPSTR lpString,
    __in int nCount,
    __in int nIDListBox);
WINUSERAPI
BOOL
...
DlgDirSelectExW(
    __in HWND hDlg,
    __out_ecount(nCount) LPWSTR lpString,
    __in int nCount,
    __in int nIDListBox);
#ifdef UNICODE
...
#if(WINVER >= 0x0400)
WINUSERAPI WORD    WINAPI TileWindows( __in __opt HWND hwndParent, __in UINT wHow, __in __opt CONST RECT * lpRect, __in UINT cKids, __in_ecount(cKids) __opt const HWND FAR * lpKids);
WINUSERAPI WORD    WINAPI CascadeWindows( __in __opt HWND hwndParent, __in UINT wHow, __in __opt CONST RECT * lpRect, __in UINT cKids,  __in_ecount(cKids) __opt const HWND FAR * lpKids);
#endif /* WINVER >= 0x0400 */
#endif /* !NOMDI */
...
WINAPI
WinHelpA(
    __in __opt HWND hWndMain,
    __in __opt LPCSTR lpszHelp,
    __in UINT uCommand,
    __in __opt ULONG_PTR dwData
    );
WINUSERAPI
...
#define UWT_3DMATRIX    0x00000001
...
#define SPI_SETSTYLUSHOTTRACKING            0x1011
#endif /* _WIN32_WINNT >= 0x0501 */
...
#define SPI_GETLEFTSCROLLBARS               0x1028
#define SPI_SETLEFTSCROLLBARS               0x1029
#endif /* _WIN32_WINNT >= 0x0501 */
...
#define DISP_CHANGE_NO_CHANGE        2
...
ChangeDisplaySettingsA(
    __in __opt LPDEVMODEA  lpDevMode,
    __in DWORD dwFlags);
WINUSERAPI
LONG
WINAPI
ChangeDisplaySettingsW(
    __in __opt LPDEVMODEW  lpDevMode,
    __in DWORD dwFlags);
#ifdef UNICODE
...
ChangeDisplaySettingsExA(
    __in __opt LPCSTR lpszDeviceName,
    __in __opt LPDEVMODEA lpDevMode,
    __in __opt HWND hwnd,
    __in DWORD dwflags,
    __in __opt LPVOID lParam);
WINUSERAPI
LONG
WINAPI
ChangeDisplaySettingsExW(
    __in __opt LPCWSTR lpszDeviceName,
    __in __opt LPDEVMODEW lpDevMode,
    __in __opt HWND hwnd,
    __in DWORD dwflags,
    __in __opt LPVOID lParam);
...
WINAPI
EnumDisplayDevicesA(
    __in __opt LPCSTR lpDevice,
    __in DWORD iDevNum,
    __out PDISPLAY_DEVICEA lpDisplayDevice,
    __in DWORD dwFlags);
WINUSERAPI
BOOL
WINAPI
EnumDisplayDevicesW(
    __in __opt LPCWSTR lpDevice,
    __in DWORD iDevNum,
    __out PDISPLAY_DEVICEW lpDisplayDevice,
    __in DWORD dwFlags);
#ifdef UNICODE
...
EnumDisplayMonitors(
    __in __opt HDC hdc,
    __in __opt LPCRECT lprcClip,
    __in MONITORENUMPROC lpfnEnum,
    __in __opt LPARAM dwData);
...
SetWinEventHook(
    __in DWORD        eventMin,
    __in DWORD        eventMax,
    __in __opt HMODULE      hmodWinEventProc,
...
#ifdef DC_ACCUMULATE
#define EVENT_SYSTEM_PAINT              0x0019
#endif
...
GetGUIThreadInfo(
    __in DWORD idThread,
    __out PGUITHREADINFO pgui);

WINUSERAPI
BOOL
WINAPI
SetConsoleFullscreen(
    __in BOOL bFullscreen,
    __in HWND hwnd);
...
GetWindowModuleFileNameA(
    __in HWND     hwnd,
    __out_ecount(cchFileNameMax) LPSTR pszFileName,
...
GetWindowModuleFileNameW(
    __in HWND     hwnd,
    __out_ecount(cchFileNameMax) LPWSTR pszFileName,
    __in UINT     cchFileNameMax);
#ifdef UNICODE
...
RealGetWindowClassA(
    __in HWND  hwnd,
    __out_ecount(cchType) __opt LPSTR pszType,
    __in UINT  cchType
);
/*
 * This gets the name of the window TYPE, not class.  This allows us to
...
RealGetWindowClassW(
    __in HWND  hwnd,
    __out_ecount(cchType) __opt LPWSTR pszType,
    __in UINT  cchType
);
#ifdef UNICODE
...
GetAltTabInfoA(
    __in __opt HWND hwnd,
    __in int iItem,
    __out PALTTABINFO pati,
    __out_ecount(cchItemText) __opt LPSTR pszItemText,
...
GetAltTabInfoW(
    ...
    __in int iItem,
    __out PALTTABINFO pati,
    __out_ecount(cchItemText) __opt LPWSTR pszItemText,
    __in UINT cchItemText
);
#ifdef UNICODE
...
/*
 * If usButtonFlags has RI_MOUSE_WHEEL or RI_MOUSE_HWHEEL, the wheel delta
 * is stored in usButtonData. Take it as a signed value.
 */
...
typedef struct tagRAWKEYBOARD {
    ...
    //
    // This field is going to be taken up to pass key pressure
...
/*
 * Bitmask for testing the PressureExtended Field to see if the value in
 * the PressureData field is a new make or just a pressure update.
*/
#define PRESSURE_UPDATE_FLAG    0x80

/*
 * Macros to use for testing if a make is a pressure update or a new make
 * and getting/setting the pressure data in the KEYBOARD_INPUT_DATA structure
 * These should be the only methods of access as this may change in future.
*/
#define IS_KEY_PRESSURE_UPDATE(pkid) (((pkid)->PressureExtended & PRESSURE_UPDATE_FLAG) ? TRUE : FALSE)
#define GET_KEY_PRESSURE_DATA(pkid) ((pkid)->PressureData)
#define SET_KEY_PRESSURE_DATA(pkid, val) ((pkid)->PressureData = (val))
...
GetRawInputData(
    ...
    __in UINT         uiCommand,
    __out_bcount(*pcbSize) __opt LPVOID      pData,
...
WINAPI
GetRawInputBuffer(
    __out_bcount(*pcbSize) __opt PRAWINPUT   pData,
    __inout PUINT    pcbSize,
    __in UINT         cbSizeHeader);
...
#define RIDEV_KEYPRESSURE       0x00000800  // effective for keyboard.
#if(_WIN32_WINNT >= 0x0501)
...
GetRegisteredRawInputDevices(
    __out_ecount( *puiNumDevices) __opt PRAWINPUTDEVICE pRawInputDevices,
...
GetRawInputDeviceList(
    __out_ecount(*puiNumDevices) __opt PRAWINPUTDEVICELIST pRawInputDeviceList,
    __inout PUINT puiNumDevices,
    __in UINT cbSize);
