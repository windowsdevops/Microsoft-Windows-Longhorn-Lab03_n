...
GetWindowModuleFileNameW(
    __in HWND    hwnd,
    __out_ecount(cchFileName) __opt LPWSTR  lpFileName,
    __in UINT    cchFileName
);
...
GetWindowModuleFileNameA(
    __in HWND    hwnd,
    __out_ecount(cchFileName) __opt LPSTR   lpFileName,
    __in UINT    cchFileName
);
...
SendInput(
    ...
    __in_bcount(cInputs * cbSize) LPINPUT pInputs,     // array of inputs
...
SetWinEventHook(
    __in DWORD           eventMin,
    __in DWORD           eventMax,
    __in HMODULE         hmodWinEventProc,   // Must pass this if global!
    __in WINEVENTPROC    lpfnWinEventProc,
    __in DWORD           idProcess,          // Can be zero; all processes
    __in DWORD           idThread,           // Can be zero; all threads
    __in DWORD           dwFlags
);
