...
//
// new required for NDIS 6.0
//
#define OID_GEN_REQUIRED_BACKFILL_SIZE          0x00010205  // query only
#define OID_GEN_LINK_SPEED_EX                   0x00010206  // query only
#define OID_GEN_MAX_LINK_SPEED                  0x00010207  // query only
...
#define OID_GEN_LINK_PARAMETERS                 0x0001020C  // set only

//
// mandatory for NDIS 6.0 miniports that create ports
//
#define OID_GEN_ENUMERATE_PORTS                 0x0001020D  // query only
#define OID_GEN_PORT_STATE                      0x0001020E  // query and set

#define OID_GEN_MACHINE_NAME                    0x0001021A
#define OID_GEN_RNDIS_CONFIG_PARAMETER          0x0001021B  // Set only
#define OID_GEN_VLAN_ID                         0x0001021C
#define OID_GEN_VLAN_STRIPING                   0x0001021D
...
#define OID_802_5_ADD_CURRENT_FUNCTIONAL_ADDRESS    0x02010208
#define OID_802_5_DELETE_CURRENT_FUNCTIONAL_ADDRESS 0x02010209
...
//
// the following structure is OID_GEN_LINK_SPEED_EX used to query the link speed 
// of a miniport. the results is expressed in bits per second. a value of -1
// indciates that NIC does not know the current link speed
//
...
#define NDIS_MINIPORT_CHECK_PACKET_FILTER               0x20000000
#endif // _NTDDNDIS_
...
typedef struct _NDIS_NSI_INTERFACE_INFORMATION_ROD
{
    //
    //  rod fields
    //
...
}NDIS_NSI_INTERFACE_INFORMATION_ROD, *PNDIS_NSI_INTERFACE_INFORMATION_ROD;


typedef struct _NDIS_NSI_INTERFACE_INFORMATION_ROS
{
    // ro static (does not change in the life of an interface)
    NET_IFINDEX                 ifIndex;
    NDIS_IF_COUNTED_STRING      ifDescr;
    NET_IFTYPE                  ifType;
    ...
    NDIS_IF_PHYSICAL_ADDRESS    PermanentPhysAddress;
    
}NDIS_NSI_INTERFACE_INFORMATION_ROS, *PNDIS_NSI_INTERFACE_INFORMATION_ROS;


typedef struct _NDIS_NSI_INTERFACE_INFORMATION
{
    // key
    NET_LUID                            ifName;

    // rw fields
    NDIS_NSI_INTERFACE_INFORMATION_RW   RwParameterStruct;

    // rod fields
    NDIS_NSI_INTERFACE_INFORMATION_ROD  RodParameterStruct;

    // ros fields
    NDIS_NSI_INTERFACE_INFORMATION_ROS  RosParameterStruct;
}NDIS_NSI_INTERFACE_INFORMATION, *PNDIS_NSI_INTERFACE_INFORMATION;
...
typedef struct _NDIS_NSI_INTERFACE_ENUM_ROD
{
    ...
    NET_IF_MEDIA_CONNECT_STATE  MediaConnectState;
    NET_IF_MEDIA_DUPLEX_STATE   MediaDuplexState;
} NDIS_NSI_INTERFACE_ENUM_ROD, *PNDIS_NSI_INTERFACE_ENUM_ROD;
...
}NDIS_NSI_INTERFACE_ENUM_ROS, *PNDIS_NSI_INTERFACE_ENUM_ROS;

typedef struct _NDIS_NSI_INTERFACE_ENUM
{
    // key
    NET_LUID                            ifName;

    NDIS_NSI_INTERFACE_ENUM_ROD         RodParameterStruct;

    NDIS_NSI_INTERFACE_ENUM_ROS         RosParameterStruct;

}NDIS_NSI_INTERFACE_ENUM, *PNDIS_NSI_INTERFACE_ENUM;


typedef struct _NDIS_NSI_INTERFACE_LOOKUP_ROS
{
    // ros
    NET_LUID                    ifName;
}NDIS_NSI_INTERFACE_LOOKUP_ROS, *PNDIS_NSI_INTERFACE_LOOKUP_ROS;


typedef struct _NDIS_NSI_INTERFACE_LOOKUP
{
    // key
    NET_IFINDEX                     ifIndex;
    // ros
    NDIS_NSI_INTERFACE_LOOKUP_ROS   RosParameterStruct;
}NDIS_NSI_INTERFACE_LOOKUP, *PNDIS_NSI_INTERFACE_LOOKUP;


typedef struct _NDIS_NSI_IF_RCV_ADDRESS_ROD
{
    // the address itself is part of the key

    // rod
    NET_IF_RCV_ADDRESS_TYPE     ifRcvAddressType;
    USHORT                      ifRcvAddressLength;
} NDIS_NSI_IF_RCV_ADDRESS_ROD, *PNDIS_NSI_IF_RCV_ADDRESS_ROD;

#define NDIS_NSI_MAX_RCV_ADDRESS_LENGTH 32
typedef struct _NDIS_NSI_IF_RCV_ADDRESS
{
    // key
    NET_LUID                    ifName;
    UCHAR                       ifRcvAddress[NDIS_NSI_MAX_RCV_ADDRESS_LENGTH]; //  0 padded
    // rod
    NDIS_NSI_IF_RCV_ADDRESS_ROD RodParameterStruct;
} NDIS_NSI_IF_RCV_ADDRESS, *PNDIS_NSI_IF_RCV_ADDRESS;


typedef struct _NDIS_NSI_IF_STACK_ENTRY
{
    // key
    NET_IFINDEX                 HigherLayerIfIndex;
    NET_IFINDEX                 LowerLayerIfIndex;
}NDIS_NSI_IF_STACK_ENTRY, *PNDIS_NSI_IF_STACK_ENTRY;

typedef struct _NDIS_NSI_INVERTED_IF_STACK_ENTRY
{
    // key
    NET_IFINDEX                 LowerLayerIfIndex;
    NET_IFINDEX                 HigherLayerIfIndex;
}NDIS_NSI_INVERTED_IF_STACK_ENTRY, *PNDIS_NSI_INVERTED_IF_STACK_ENTRY;
...
typedef struct _NDIS_NSI_NETWORK_RW
{
    ...
    NET_IF_COMPARTMENT_ID       CompartmentId;
    ULONG                       SiteId;
    NDIS_IF_COUNTED_STRING      NetworkName;
}NDIS_NSI_NETWORK_RW, *PNDIS_NSI_NETWORK_RW;

typedef struct _NDIS_NSI_NETWORK
{
    // key
    NET_IF_NETWORK_GUID         NetworkGuid;
    // RW fields
    NDIS_NSI_NETWORK_RW    RwParameterStruct;
}NDIS_NSI_NETWORK, *PNDIS_NSI_NETWORK;


typedef struct _NDIS_NSI_COMPARTMENT_RW
{
    // ros field
    NET_LUID                    LoopbackIfLuid;
    NET_IF_NETWORK_GUID         LoopbackIfNetworkGuid;
    GUID                        LoopbackIfInterfaceGuid;
    NDIS_IF_COUNTED_STRING      LoopbackIfDescr;
    NDIS_IF_COUNTED_STRING      LoopbackIfAlias;
}NDIS_NSI_COMPARTMENT_RW, *PNDIS_NSI_COMPARTMENT_RW;


typedef struct _NDIS_NSI_COMPARTMENT
{
    // key
    NET_IF_COMPARTMENT_ID               CompartmentId;
    // rw field for NDIS, should not be set by any NSI client
    NDIS_NSI_COMPARTMENT_RW             RwParameterStruct;
}NDIS_NSI_COMPARTMENT, *PNDIS_NSI_COMPARTMENT;
...
typedef struct _NDIS_NSI_THREAD_RW {
    NET_IF_COMPARTMENT_ID CompartmentId;
} NDIS_NSI_THREAD_RW, *PNDIS_NSI_THREAD_RW;

typedef struct _NDIS_NSI_SESSION_KEY {
    ULONG SessionId;
} NDIS_NSI_SESSION_KEY, *PNDIS_NSI_SESSION_KEY;

typedef struct _NDIS_NSI_SESSION_RW {
    NET_IF_COMPARTMENT_ID CompartmentId;
} NDIS_NSI_SESSION_RW, *PNDIS_NSI_SESSION_RW;
