...
//
// API Build Version Information
//

//
// APIs declared in this header file are available starting from
// Windows Server 2003.  They can not be used on older systems.
//

#if _WIN32_WINNT < 0x0502
#error "HTTP.H requires Windows 2003 SDK or higher!"
#endif
...
//
// This is for setting configuration on a Request Queue Handle -
// Via HttpSetRequestQueueProperty & HttpQueryRequestQueueProperty.
//

typedef enum _HTTP_REQUEST_QUEUE_PROPERTY
{
    //
    // Query/Set the Authentication information for this Request Queue.
    //
    // pReqQueueInformation points to a
    // HTTP_REQUEST_QUEUE_AUTHENTICATION structure.
    //

    HttpRequestQueueAuthProperty,

    //
    // Query the default network connections limit.
    //
    // pControlChannelInformation points to a
    // HTTP_CONNECTION_LIMIT value.
    //

    HttpRequestQueueConnectionProperty,

    //
    // Query/Set the Connection Timeout information.
    //
    // pReqQueueInformation points to a
    // HTTP_REQUEST_QUEUE_TIMEOUT_LIMIT structure.
    //

    HttpRequestQueueConnectionTimeoutProperty,

    //
    // Query/set the maximum number of queued new requests on
    // the Request Queue.
    //
    // pReqQueueInformation points to a LONG containing the maximum
    // number of queued requests.
    //

    HttpRequestQueueLengthProperty,

    //
    // Set the logging related settings.
    //
    // pReqQueueInformation points to a
    // HTTP_REQUEST_QUEUE_LOGGING structure.
    //

    HttpRequestQueueLoggingProperty,

    //
    // Query/set the enabled state of the Request Queue.
    //
    // pReqQueueInformation points to an
    // HTTP_ENABLED_STATE enum value.
    //

    HttpRequestQueueStateProperty,

    HttpRequestQueuePropertyMax

} HTTP_REQUEST_QUEUE_PROPERTY, *PHTTP_REQUEST_QUEUE_PROPERTY;
...
typedef struct _HTTP_REQUEST_QUEUE_AUTHENTICATION
{
...
} HTTP_REQUEST_QUEUE_AUTHENTICATION, *PHTTP_REQUEST_QUEUE_AUTHENTICATION;

//
// HTTP_REQUEST_QUEUE_LOGGING
//
...
//
// HTTP_REQUEST_QUEUE_TIMEOUT_LIMIT
//

typedef struct _HTTP_REQUEST_QUEUE_TIMEOUT_LIMIT
{
    HTTP_PROPERTY_FLAGS     Flags;
    ULONG                   ConnectionTimeout;  // Seconds
    ULONG                   HeaderWaitTimeout;  // Seconds
    ULONG                   MinFileKbSec;       // Bytes/Second
} HTTP_REQUEST_QUEUE_TIMEOUT_LIMIT, *PHTTP_REQUEST_QUEUE_TIMEOUT_LIMIT;
...
//
// Http request authentication type is used client and
// server side authentication. However types AutoSelect
// and Kerberos are not used by the server side.
//
...
typedef enum _HTTP_REQUEST_AUTH_TYPE
{
    ...
    //
    // HttpRequestAuthTypeAutoSelect can be used to let the
    // client side API select the "strongest" scheme that
    // is supported by the server.  For security reasons,
    // the API does not select BASIC scheme.
    //

    HttpRequestAuthTypeAutoSelect,
...
//
// The structure of an HTTP request for Win2003 Server
// This structure is always defined
//
...
{
    HttpResponseInfoTypeDigest,
    HttpResponseInfoTypeMax
...
{
    struct _HTTP_RESPONSE_V1;        // Anonymous structure
...
//
// HTTP_RESPONSE_FLAG_MORE_ENTITY_BODY_EXISTS - When this flag is returned by
// HTTP Client APIs, it indicates that there is more entity body in the
// response.  It must be read using HttpReceiveResponseEntityBody API.
//

#define HTTP_RESPONSE_FLAG_MORE_ENTITY_BODY_EXISTS  0x00000001
...
ULONG
WINAPI
HttpCreateNamedHttpHandle(
    OUT PHANDLE   pReqQueueHandle,
    IN  PCWSTR    ReqQueueName,
...
HttpSetRequestQueueProperty(
    IN HANDLE ReqQueueHandle,
    IN HTTP_REQUEST_QUEUE_PROPERTY ReqQueueProperty,
    IN PVOID pReqQueueInformation,
    IN ULONG Length,
