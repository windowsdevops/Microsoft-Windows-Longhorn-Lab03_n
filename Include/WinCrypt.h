...
#define CERT_FIRST_RESERVED_PROP_ID         71
...
#define CRYPTNET_URL_CACHE_PRE_FETCH_AUTOROOT_TXT   4
...
#define CRYPTNET_URL_CACHE_DEFAULT_FLUSH_EXEMPT_SECONDS \
            (60 * 60 * 24 * 14)
...
// The following are REG_DWORD's. These configuration parameters are
// used by the Cryptnet Url Cache Service (CUCS) for calculating
// random PreFetch times.

// Following 3 parameters are used if the PublishTime isn't specified
// in the URL object to be PreFetch'ed.

// An initial guesstimate of the publish period before NextUpdate
// is to divide the ValidityPeriod (NextUpdate - ThisUpdate) by this integer
// divisor.
#define CRYPTNET_PRE_FETCH_VALIDITY_PERIOD_PUBLISHER_DIVISOR_VALUE_NAME \
    L"CryptnetPreFetchValidityPeriodPublishDivisor"
// 10, where 1 week / 10 = 16.8 hours or 12 hours / 10 = 1.2 hours
#define CRYPTNET_PRE_FETCH_VALIDITY_PERIOD_PUBLISHER_DIVISOR_DEFAULT \
    10

// If necessary, the above guesstimated publish period will be decreased
// to this maximum duration in seconds.
#define CRYPTNET_PRE_FETCH_MAX_PUBLISH_PERIOD_SECONDS_VALUE_NAME \
    L"CryptnetPreFetchMaxPublishPeriodSeconds"
// 12 hours
#define CRYPTNET_PRE_FETCH_MAX_PUBLISH_PERIOD_SECONDS_DEFAULT \
    (12 * 60 * 60)

// If necessary, the above guesstimated publish period will be increased
// to this minimum duration in seconds.
#define CRYPTNET_PRE_FETCH_MIN_PUBLISH_PERIOD_SECONDS_VALUE_NAME \
    L"CryptnetPreFetchMinPublishPeriodSeconds"
// 25 minutes
#define CRYPTNET_PRE_FETCH_MIN_PUBLISH_PERIOD_SECONDS_DEFAULT \
    (25 * 60)
...
// Following 2 parameters are used when reconnecting

// When disconnected from the internet with a pending URL retrieval, 
// the following parameter specifies the wait time between pollings for
// a restored connection.
#define CRYPTNET_PRE_FETCH_NOT_CONNECTED_POLL_SECONDS_VALUE_NAME \
    L"CryptnetPreFetchNotConnectedPollSeconds"
// 5 minutes
#define CRYPTNET_PRE_FETCH_NOT_CONNECTED_POLL_SECONDS_DEFAULT \
    (5 * 60)

// Following parameter specifies the delay time to wait after a connection
// is restored before doing a pending URL retrieval.
#define CRYPTNET_PRE_FETCH_AFTER_CONNECTED_DELAY_SECONDS_VALUE_NAME \
    L"CryptnetPreFetchAfterConnectedDelaySeconds"
// 1 minutes
#define CRYPTNET_PRE_FETCH_AFTER_CONNECTED_DELAY_SECONDS_DEFAULT \
    (1 * 60)


// Following 2 parameters are passed to the Background Intelligent
// Transfer Service (BITS) to control the times it uses for transient errors.

#define CRYPTNET_PRE_FETCH_BITS_NO_PROGRESS_TIMEOUT_VALUE_NAME \
    L"CryptnetPreFetchBitsNoProgressTimeout"
// 5 minutes
#define CRYPTNET_PRE_FETCH_BITS_NO_PROGRESS_TIMEOUT_DEFAULT \
    (5 * 60)

#define CRYPTNET_PRE_FETCH_BITS_MINIMUM_RETRY_DELAY_VALUE_NAME \
    L"CryptnetPreFetchBitsMinimumRetryDelay"
// 1 minute
#define CRYPTNET_PRE_FETCH_BITS_MINIMUM_RETRY_DELAY_DEFAULT \
    (1 * 60)

// Following parameter specifies the delay time to wait to process a
// queued logon notification including service start LocalSystem, LocalService,
// and NetworkService logons
#define CRYPTNET_PRE_FETCH_LOGON_DELAY_SECONDS_VALUE_NAME \
    L"CryptnetPreFetchLogonDelaySeconds"
// 1.5 minutes
#define CRYPTNET_PRE_FETCH_LOGON_DELAY_SECONDS_DEFAULT \
    (90)

#define CRYPTNET_PRE_FETCH_LOGON_DELAY_SECONDS_DISABLE \
    0xFFFFFFFF
...
#define PKCS12_NO_PERSIST_KEY          0x00008000 // Private key will not be persisted
...
WINAPI
CertFreeServerOcspResponseContext(
    IN OUT PCCERT_SERVER_OCSP_RESPONSE_CONTEXT pServerOcspResponseContext
    );
...
CertRetrieveLogoOrBiometricInfo(
    ...
    IN DWORD dwFlags,
    IN OPTIONAL void *pvReserved,
    OUT BYTE **ppbData,                     // CryptMemFree()
    OUT DWORD *pcbData,
    OUT OPTIONAL LPWSTR *ppwszMimeType      // CryptMemFree()
    );
