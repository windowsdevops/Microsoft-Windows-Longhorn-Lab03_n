...
LWSTDAPI_(LPSTR)    StrNCatA(__in LPSTR psz1, LPCSTR psz2, int cchMax);
LWSTDAPI_(LPWSTR)   StrNCatW(__in LPWSTR psz1, LPCWSTR psz2, int cchMax);
#ifdef USE_STRICT_CONST
...
LWSTDAPI_(LPCWSTR)   StrRChrW(LPCWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch);
LWSTDAPI_(LPCSTR)    StrRChrIA(LPCSTR lpStart, LPCSTR lpEnd, WORD wMatch);
LWSTDAPI_(LPCWSTR)   StrRChrIW(LPCWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch);
...
LWSTDAPI_(LPWSTR)   StrRChrW(LPCWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch);
LWSTDAPI_(LPSTR)    StrRChrIA(LPCSTR lpStart, LPCSTR lpEnd, WORD wMatch);
LWSTDAPI_(LPWSTR)   StrRChrIW(LPCWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch);
...
LWSTDAPI_(BOOL)     StrToIntExA(LPCSTR pszString, DWORD dwFlags, __in int * piRet);
LWSTDAPI_(BOOL)     StrToIntExW(LPCWSTR pszString, DWORD dwFlags, __in int * piRet);
#if (_WIN32_IE >= 0x0600)
LWSTDAPI_(BOOL)     StrToInt64ExA(LPCSTR pszString, DWORD dwFlags, __out LONGLONG * pllRet);
LWSTDAPI_(BOOL)     StrToInt64ExW(LPCWSTR pszString, DWORD dwFlags, __out LONGLONG * pllRet);
#endif
...
LWSTDAPI StrRetToStrA(__in STRRET *pstr, __in __opt PCUITEMID_CHILD pidl, __out LPSTR *ppsz);
LWSTDAPI StrRetToStrW(__in STRRET *pstr, __in __opt PCUITEMID_CHILD pidl, __out LPWSTR *ppsz);
#ifdef UNICODE
#define StrRetToStr  StrRetToStrW
#else
#define StrRetToStr  StrRetToStrA
#endif // !UNICODE
LWSTDAPI StrRetToBufA(__in STRRET *pstr, __in __opt PCUITEMID_CHILD pidl, __out_ecount(cchBuf) LPSTR pszBuf, UINT cchBuf);
LWSTDAPI StrRetToBufW(__in STRRET *pstr, __in __opt PCUITEMID_CHILD pidl, __out_ecount(cchBuf) LPWSTR pszBuf, UINT cchBuf);
#ifdef UNICODE
...
LWSTDAPI SHStrDupA(LPCSTR psz, __out WCHAR **ppwsz);
LWSTDAPI SHStrDupW(LPCWSTR psz, __out WCHAR **ppwsz);
#ifdef UNICODE
...
inline HRESULT SHLocalStrDupW(LPCWSTR psz, LPWSTR *ppsz)
{
    *ppsz = StrDupW(psz);
    return *ppsz ? S_OK : E_OUTOFMEMORY;
}
inline HRESULT SHLocalStrDupA(LPCSTR psz, LPSTR *ppsz)
{
    *ppsz = StrDupA(psz);
    return *ppsz ? S_OK : E_OUTOFMEMORY;
}
...
LWSTDAPI StrRetToBSTR(__in STRRET *pstr, __in PCUITEMID_CHILD pidl, __out BSTR *pbstr);
LWSTDAPI SHLoadIndirectString(LPCWSTR pszSource, __out_ecount(cchOutBuf) LPWSTR pszOutBuf, UINT cchOutBuf, __in __opt void **ppvReserved);
#endif // _WIN32_IE >= 0x0550
...
LWSTDAPI_(LPSTR)  PathBuildRootA(__in_ecount(4) LPSTR pszRoot, int iDrive);
LWSTDAPI_(LPWSTR)  PathBuildRootW(__in_ecount(4) LPWSTR pszRoot, int iDrive);
#ifdef UNICODE
...
LWSTDAPI_(int)      PathCommonPrefixA(LPCSTR pszFile1, LPCSTR pszFile2, __in __opt LPSTR achPath);
LWSTDAPI_(int)      PathCommonPrefixW(LPCWSTR pszFile1, LPCWSTR pszFile2, __in __opt LPWSTR achPath);
...
#endif // !UNICODE
#endif
LWSTDAPI_(BOOL)     PathFindOnPathA(__in LPSTR pszPath, LPCSTR * ppszOtherDirs);
LWSTDAPI_(BOOL)     PathFindOnPathW(__in LPWSTR pszPath, LPCWSTR * ppszOtherDirs);
...
LWSTDAPI_(BOOL)     PathQuoteSpacesA(__inout LPSTR lpsz);
LWSTDAPI_(BOOL)     PathQuoteSpacesW(__inout LPWSTR lpsz);
...
LWSTDAPI_(BOOL)     PathRenameExtensionA(__inout LPSTR pszPath, LPCSTR pszExt);
LWSTDAPI_(BOOL)     PathRenameExtensionW(__inout LPWSTR pszPath, LPCWSTR pszExt);
...
#endif // !UNICODE
LWSTDAPI_(BOOL)     PathStripToRootA(__in LPSTR pszPath);
LWSTDAPI_(BOOL)     PathStripToRootW(__in LPWSTR pszPath);
#ifdef UNICODE
...
LWSTDAPI                UrlCombineA(LPCSTR pszBase, LPCSTR pszRelative, __out_ecount(*pcchCombined) __opt LPSTR pszCombined, __inout LPDWORD pcchCombined, DWORD dwFlags);
LWSTDAPI                UrlCombineW(LPCWSTR pszBase, LPCWSTR pszRelative, __out_ecount(*pcchCombined) __opt LPWSTR pszCombined, __inout LPDWORD pcchCombined, DWORD dwFlags);
...
LWSTDAPI                UrlUnescapeA(__inout LPSTR pszUrl, __out_ecount(*pcchUnescaped) __opt LPSTR pszUnescaped, __inout __opt LPDWORD pcchUnescaped, DWORD dwFlags);
LWSTDAPI                UrlUnescapeW(__inout LPWSTR pszUrl, __out_ecount(*pcchUnescaped) __opt LPWSTR pszUnescaped, __inout __opt LPDWORD pcchUnescaped, DWORD dwFlags);
...
LWSTDAPI                HashData(__in LPBYTE pbData, DWORD cbData, __out_ecount(cbHash) LPBYTE pbHash, DWORD cbHash);
...
LWSTDAPI_(DWORD)    SHDeleteEmptyKeyA(__in HKEY hkey, LPCSTR pszSubKey);
LWSTDAPI_(DWORD)    SHDeleteEmptyKeyW(__in HKEY hkey, LPCWSTR pszSubKey);
#ifdef UNICODE
#define SHDeleteEmptyKey  SHDeleteEmptyKeyW
#else
#define SHDeleteEmptyKey  SHDeleteEmptyKeyA
#endif // !UNICODE
LWSTDAPI_(DWORD)    SHDeleteKeyA(__in HKEY hkey, LPCSTR pszSubKey);
LWSTDAPI_(DWORD)    SHDeleteKeyW(__in HKEY hkey, LPCWSTR pszSubKey);
#ifdef UNICODE
...
LWSTDAPI_(DWORD)    SHDeleteValueA(__in HKEY hkey, LPCSTR pszSubKey, LPCSTR pszValue);
LWSTDAPI_(DWORD)    SHDeleteValueW(__in HKEY hkey, LPCWSTR pszSubKey, LPCWSTR pszValue);
#ifdef UNICODE
#define SHDeleteValue  SHDeleteValueW
#else
#define SHDeleteValue  SHDeleteValueA
#endif // !UNICODE
LWSTDAPI_(DWORD)    SHGetValueA(__in HKEY hkey, LPCSTR pszSubKey, LPCSTR pszValue, __out __opt DWORD *pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt DWORD *pcbData);
LWSTDAPI_(DWORD)    SHGetValueW(__in HKEY hkey, LPCWSTR pszSubKey, LPCWSTR pszValue, __out __opt DWORD *pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt DWORD *pcbData);
#ifdef UNICODE
...
#endif // !UNICODE
LWSTDAPI_(DWORD)    SHSetValueA(__in HKEY hkey, LPCSTR pszSubKey, LPCSTR pszValue, DWORD dwType,
    __in_bcount(cbData) __opt LPCVOID pvData, DWORD cbData);
LWSTDAPI_(DWORD)    SHSetValueW(__in HKEY hkey, LPCWSTR pszSubKey, LPCWSTR pszValue, DWORD dwType,
    __in_bcount(cbData) __opt LPCVOID pvData, DWORD cbData);
#ifdef UNICODE
...
LWSTDAPI_(LONG)     SHRegGetValueA(__in HKEY hkey, LPCSTR pszSubKey, LPCSTR pszValue,
    __in SRRF dwFlags, __out __opt DWORD *pdwType, __typefix(LPBYTE) __out_bcount(*pcbData) __opt void *pvData,
    __inout __opt DWORD *pcbData);
LWSTDAPI_(LONG)     SHRegGetValueW(__in HKEY hkey, LPCWSTR pszSubKey, LPCWSTR pszValue,
    __in SRRF dwFlags, __out __opt DWORD *pdwType, __typefix(LPBYTE) __out_bcount(*pcbData) __opt void *pvData,
    __inout __opt DWORD *pcbData);
#ifdef UNICODE
...
LWSTDAPI_(DWORD)    SHQueryValueExA(__in HKEY hkey, LPCSTR pszValue, __in __opt DWORD *pdwReserved, __out __opt DWORD *pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __out __opt DWORD *pcbData);
LWSTDAPI_(DWORD)    SHQueryValueExW(__in HKEY hkey, LPCWSTR pszValue, __in __opt DWORD *pdwReserved, __out __opt DWORD *pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __out __opt DWORD *pcbData);
#ifdef UNICODE
...
LWSTDAPI_(LONG)     SHEnumKeyExA(__in HKEY hkey, DWORD dwIndex, __out_ecount(*pcchName) LPSTR pszName, __inout LPDWORD pcchName);
LWSTDAPI_(LONG)     SHEnumKeyExW(__in HKEY hkey, DWORD dwIndex, __out_ecount(*pcchName) LPWSTR pszName, __inout LPDWORD pcchName);
LWSTDAPI_(LONG)     SHEnumValueA(__in HKEY hkey, DWORD dwIndex, __out_ecount(*pcchValueName) __opt LPSTR pszValueName,
    __inout __opt LPDWORD pcchValueName, __out __opt LPDWORD pdwType, __out_bcount(*pcbData) __opt void *pvData,
    __inout __opt LPDWORD pcbData);
LWSTDAPI_(LONG)     SHEnumValueW(__in HKEY hkey, DWORD dwIndex, __out_ecount(*pcchValueName) __opt LPWSTR pszValueName,
    __inout __opt LPDWORD pcchValueName, __out __opt LPDWORD pdwType, __out_bcount(*pcbData) __opt void *pvData,
    __inout __opt LPDWORD pcbData);
LWSTDAPI_(LONG)     SHQueryInfoKeyA(__in HKEY hkey, __out __opt LPDWORD pcSubKeys, __out __opt LPDWORD pcchMaxSubKeyLen, __out __opt LPDWORD pcValues, __out __opt LPDWORD pcchMaxValueNameLen);
LWSTDAPI_(LONG)     SHQueryInfoKeyW(__in HKEY hkey, __out __opt LPDWORD pcSubKeys, __out __opt LPDWORD pcchMaxSubKeyLen, __out __opt LPDWORD pcValues, __out __opt LPDWORD pcchMaxValueNameLen);

// recursive key copy
LWSTDAPI_(DWORD)     SHCopyKeyA(__in HKEY hkeySrc, LPCSTR   szSrcSubKey, __in HKEY hkeyDest, DWORD fReserved);
LWSTDAPI_(DWORD)     SHCopyKeyW(__in HKEY hkeySrc, LPCWSTR wszSrcSubKey, __in HKEY hkeyDest, DWORD fReserved);

// Getting and setting file system paths with environment variables

LWSTDAPI_(DWORD)    SHRegGetPathA(__in HKEY hKey, LPCSTR pcszSubKey, LPCSTR pcszValue, __out_ecount(MAX_PATH) LPSTR pszPath, DWORD dwFlags);
LWSTDAPI_(DWORD)    SHRegGetPathW(__in HKEY hKey, LPCWSTR pcszSubKey, LPCWSTR pcszValue, __out_ecount(MAX_PATH) LPWSTR pszPath, DWORD dwFlags);
LWSTDAPI_(DWORD)    SHRegSetPathA(__in HKEY hKey, LPCSTR pcszSubKey, LPCSTR pcszValue, LPCSTR pcszPath, DWORD dwFlags);
LWSTDAPI_(DWORD)    SHRegSetPathW(__in HKEY hKey, LPCWSTR pcszSubKey, LPCWSTR pcszValue, LPCWSTR pcszPath, DWORD dwFlags);
...
LWSTDAPI_(LONG)        SHRegCreateUSKeyA(LPCSTR pszPath, __in REGSAM samDesired, HUSKEY hRelativeUSKey, __out PHUSKEY phNewUSKey, DWORD dwFlags);
LWSTDAPI_(LONG)        SHRegCreateUSKeyW(LPCWSTR pwzPath, __in REGSAM samDesired, HUSKEY hRelativeUSKey, __out PHUSKEY phNewUSKey, DWORD dwFlags);
LWSTDAPI_(LONG)        SHRegOpenUSKeyA(LPCSTR pszPath, __in REGSAM samDesired, HUSKEY hRelativeUSKey, __out PHUSKEY phNewUSKey, BOOL fIgnoreHKCU);
LWSTDAPI_(LONG)        SHRegOpenUSKeyW(LPCWSTR pwzPath, __in REGSAM samDesired, HUSKEY hRelativeUSKey, __out PHUSKEY phNewUSKey, BOOL fIgnoreHKCU);
LWSTDAPI_(LONG)        SHRegQueryUSValueA(__in HUSKEY hUSKey, LPCSTR pszValue, __out __opt LPDWORD pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt LPDWORD pcbData, BOOL fIgnoreHKCU,
    __in_bcount(dwDefaultDataSize) __opt void *pvDefaultData, DWORD dwDefaultDataSize);
LWSTDAPI_(LONG)        SHRegQueryUSValueW(__in HUSKEY hUSKey, __out LPCWSTR pwzValue, __out __opt LPDWORD pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt LPDWORD pcbData, BOOL fIgnoreHKCU,
    __in_bcount(dwDefaultDataSize) __opt void *pvDefaultData, DWORD dwDefaultDataSize);
LWSTDAPI_(LONG)        SHRegWriteUSValueA(__in HUSKEY hUSKey, LPCSTR pszValue, DWORD dwType, __in_bcount(cbData) const void *pvData, DWORD cbData, DWORD dwFlags);
LWSTDAPI_(LONG)        SHRegWriteUSValueW(__in HUSKEY hUSKey, LPCWSTR pwzValue, DWORD dwType, __in_bcount(cbData) const void *pvData, DWORD cbData, DWORD dwFlags);
LWSTDAPI_(LONG)        SHRegDeleteUSValueA(__in HUSKEY hUSKey, LPCSTR pszValue, __in SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LONG)        SHRegDeleteEmptyUSKeyW(__in HUSKEY hUSKey, LPCWSTR pwzSubKey, __in SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LONG)        SHRegDeleteEmptyUSKeyA(__in HUSKEY hUSKey, LPCSTR pszSubKey, __in SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LONG)        SHRegDeleteUSValueW(__in HUSKEY hUSKey, LPCWSTR pwzValue, __in SHREGDEL_FLAGS delRegFlags);
LWSTDAPI_(LONG)        SHRegEnumUSKeyA(__in HUSKEY hUSKey, DWORD dwIndex, __out_ecount(*pcchName) LPSTR pszName, __inout LPDWORD pcchName, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegEnumUSKeyW(__in HUSKEY hUSKey, DWORD dwIndex, __out_ecount(*pcchName) LPWSTR pwzName, __inout LPDWORD pcchName, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegEnumUSValueA(__in HUSKEY hUSkey, DWORD dwIndex,
    __out_ecount(*pcchValueName) LPSTR pszValueName, __inout LPDWORD pcchValueName, __out __opt LPDWORD pdwType, 
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt LPDWORD pcbData, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegEnumUSValueW(__in HUSKEY hUSkey, DWORD dwIndex,
    __out_ecount(*pcchValueName) LPWSTR pszValueName, __inout LPDWORD pcchValueName, __out __opt LPDWORD pdwType,
    __out_bcount(*pcbData) __opt void *pvData, __inout __opt LPDWORD pcbData, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegQueryInfoUSKeyA(__in HUSKEY hUSKey, __out __opt LPDWORD pcSubKeys, __out __opt LPDWORD pcchMaxSubKeyLen, __out __opt LPDWORD pcValues, __out __opt LPDWORD pcchMaxValueNameLen, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegQueryInfoUSKeyW(__in HUSKEY hUSKey, __out __opt LPDWORD pcSubKeys, __out __opt LPDWORD pcchMaxSubKeyLen, __out __opt LPDWORD pcValues, __out __opt LPDWORD pcchMaxValueNameLen, __in SHREGENUM_FLAGS enumRegFlags);
LWSTDAPI_(LONG)        SHRegCloseUSKey(__in HUSKEY hUSKey);


// These calls are equal to an SHRegOpenUSKey, SHRegQueryUSValue, and then a SHRegCloseUSKey.
LWSTDAPI_(LONG)        SHRegGetUSValueA(LPCSTR pszSubKey, LPCSTR pszValue, __out __opt LPDWORD pdwType,
    __out_bcount(*pcbData) __opt void * pvData, __inout __opt LPDWORD pcbData, BOOL fIgnoreHKCU,
    __in_bcount(dwDefaultDataSize) __opt void *pvDefaultData, DWORD dwDefaultDataSize);
LWSTDAPI_(LONG)        SHRegGetUSValueW(LPCWSTR pwzSubKey, LPCWSTR pwzValue, __out __opt LPDWORD pdwType,
    __out_bcount(*pcbData) __opt void * pvData, __inout __opt LPDWORD pcbData, BOOL fIgnoreHKCU,
    __in_bcount(dwDefaultDataSize) __opt void *pvDefaultData, DWORD dwDefaultDataSize);
LWSTDAPI_(LONG)        SHRegSetUSValueA(LPCSTR pszSubKey, LPCSTR pszValue, DWORD dwType, __in_bcount(cbData) __opt const void *pvData, DWORD cbData, DWORD dwFlags);
LWSTDAPI_(LONG)        SHRegSetUSValueW(LPCWSTR pwzSubKey, LPCWSTR pwzValue, DWORD dwType, __in_bcount(cbData) __opt const void *pvData, DWORD cbData, DWORD dwFlags);
LWSTDAPI_(int)         SHRegGetIntW(__in HKEY hk, LPCWSTR pwzKey, int iDefault);
...
DECLARE_INTERFACE_IID_( IQueryAssociations, IUnknown, "c46ca590-3c3f-11d2-bee6-0000f805ca57" )
{
    ...
    STDMETHOD (GetString)(THIS_ ASSOCF flags, ASSOCSTR str, LPCWSTR pszExtra, __out_ecount(*pcchOut) __opt LPWSTR pszOut, __inout DWORD *pcchOut) PURE;
    STDMETHOD (GetKey)(THIS_ ASSOCF flags, ASSOCKEY key, LPCWSTR pszExtra, __out HKEY *phkeyOut) PURE;
    STDMETHOD (GetData)(THIS_ ASSOCF flags, ASSOCDATA data, LPCWSTR pszExtra, __out_bcount(*pcbOut) __opt LPVOID pvOut, __inout DWORD *pcbOut) PURE;
    STDMETHOD (GetEnum)(THIS_ ASSOCF flags, ASSOCENUM assocenum, LPCWSTR pszExtra, REFIID riid, __out LPVOID *ppvOut) PURE;
};


LWSTDAPI AssocCreate(__in CLSID clsid, __in REFIID riid, __out LPVOID *ppv);

//  wrappers for the interface
LWSTDAPI AssocQueryStringA(__in ASSOCF flags, __in ASSOCSTR str, LPCSTR pszAssoc, LPCSTR pszExtra, __out_ecount(*pcchOut) __opt LPSTR pszOut, __inout DWORD *pcchOut);
//  wrappers for the interface
LWSTDAPI AssocQueryStringW(__in ASSOCF flags, __in ASSOCSTR str, LPCWSTR pszAssoc, LPCWSTR pszExtra, __out_ecount(*pcchOut) __opt LPWSTR pszOut, __inout DWORD *pcchOut);
#ifdef UNICODE
#define AssocQueryString  AssocQueryStringW
#else
#define AssocQueryString  AssocQueryStringA
#endif // !UNICODE
LWSTDAPI AssocQueryStringByKeyA(__in ASSOCF flags, __in ASSOCSTR str, __in HKEY hkAssoc, LPCSTR pszExtra, __out_ecount(*pcchOut) __opt LPSTR pszOut, __inout DWORD *pcchOut);
LWSTDAPI AssocQueryStringByKeyW(__in ASSOCF flags, __in ASSOCSTR str, __in HKEY hkAssoc, LPCWSTR pszExtra, __out_ecount(*pcchOut) __opt LPWSTR pszOut, __inout DWORD *pcchOut);
#ifdef UNICODE
#define AssocQueryStringByKey  AssocQueryStringByKeyW
#else
#define AssocQueryStringByKey  AssocQueryStringByKeyA
#endif // !UNICODE
LWSTDAPI AssocQueryKeyA(__in ASSOCF flags, __in ASSOCKEY key, LPCSTR pszAssoc, LPCSTR pszExtra, __out HKEY *phkeyOut);
LWSTDAPI AssocQueryKeyW(__in ASSOCF flags, __in ASSOCKEY key, LPCWSTR pszAssoc, LPCWSTR pszExtra, __out HKEY *phkeyOut);
#ifdef UNICODE
...
LWSTDAPI SHCreateStreamOnFileEx(LPCWSTR pszFile, DWORD grfMode, DWORD dwAttributes, BOOL fCreate, __in __opt struct IStream * pstmTemplate, __out struct IStream **ppstm);
...
// SHAutoCompleteGetPidl Flags
#define SHACGPF_NONE                    0x00000000  // Defaults 
#define SHACGPF_SIMPLEPIDL              0x00000001  // Get Simple PIDL if path can't be resolved (filesave dialog)
                                                    //     as long as only the filename part of the
                                                    //     path doesn't exsist (parent does)
#define SHACGPF_VIRTUALNS_ONLY          0x00000002  // Do not search elsewhere for the path.
#define SHACGPF_DONT_PARSE_RELATIVE     0x00000004  // Do not parse relative paths.

LWSTDAPI SHAutoComplete(HWND hwndEdit, DWORD dwFlags);
LWSTDAPI SHAutoCompGetPidl(HWND hwndEdit, __in __opt PIDLIST_ABSOLUTE pidlCurDir, __out PIDLIST_ABSOLUTE* ppidl, DWORD dwFlags);

LWSTDAPI SHCreateThreadRef(__in LONG *pcRef, __out IUnknown **ppunk);
LWSTDAPI SHSetThreadRef(__in IUnknown *punk);
LWSTDAPI SHGetThreadRef(__in IUnknown **ppunk);
...
LWSTDAPI_(BOOL) SHCreateThread(__in LPTHREAD_START_ROUTINE pfnThreadProc, __in __opt void *pData, DWORD dwFlags, __in __opt LPTHREAD_START_ROUTINE pfnCallback);
LWSTDAPI_(BOOL) SHCreateThreadWithHandle(__in LPTHREAD_START_ROUTINE pfnThreadProc, __in __opt void *pData, DWORD dwFlags, __in __opt LPTHREAD_START_ROUTINE pfnCallback, __in __opt HANDLE *pHandle);
...
__inline LPSTR StrPBrkA(__in LPSTR psz, LPCSTR pszSet)
{
    return const_cast<LPSTR>(StrPBrkA(const_cast<LPCSTR>(psz), pszSet));
}
__inline LPWSTR StrPBrkW(__in LPWSTR psz, LPCWSTR pszSet)
{
    return const_cast<LPWSTR>(StrPBrkW(const_cast<LPCWSTR>(psz), pszSet));
}
__inline LPSTR StrRChrA(__in LPSTR lpStart, LPCSTR lpEnd, WORD wMatch)
{
    return const_cast<LPSTR>(StrRChrA(const_cast<LPCSTR>(lpStart), lpEnd, wMatch));
}
__inline LPWSTR StrRChrW(__in LPWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch)
{
    return const_cast<LPWSTR>(StrRChrW(const_cast<LPCWSTR>(lpStart), lpEnd, wMatch));
}
__inline LPSTR StrRChrIA(__in LPSTR lpStart, LPCSTR lpEnd, WORD wMatch)
{
    return const_cast<LPSTR>(StrRChrIA(const_cast<LPCSTR>(lpStart), lpEnd, wMatch));
}
__inline LPWSTR StrRChrIW(__in LPWSTR lpStart, LPCWSTR lpEnd, __in WCHAR wMatch)
{
    return const_cast<LPWSTR>(StrRChrIW(const_cast<LPCWSTR>(lpStart), lpEnd, wMatch));
}
__inline LPSTR StrRStrIA(__in LPSTR lpSource, LPCSTR lpLast, LPCSTR lpSrch)
{
    return const_cast<LPSTR>(StrRStrIA(const_cast<LPCSTR>(lpSource), lpLast, lpSrch));
}
__inline LPWSTR StrRStrIW(__in LPWSTR lpSource, LPCWSTR lpLast, LPCWSTR lpSrch)
{
    return const_cast<LPWSTR>(StrRStrIW(const_cast<LPCWSTR>(lpSource), lpLast, lpSrch));
}
__inline LPSTR StrStrA(__in LPSTR lpFirst, LPCSTR lpSrch)
{
    return const_cast<LPSTR>(StrStrA(const_cast<LPCSTR>(lpFirst), lpSrch));
}
__inline LPWSTR StrStrW(__in LPWSTR lpFirst, LPCWSTR lpSrch)
{
    return const_cast<LPWSTR>(StrStrW(const_cast<LPCWSTR>(lpFirst), lpSrch));
}
__inline LPWSTR StrStrNW(__in LPWSTR lpFirst, LPCWSTR lpSrch, UINT cchMax)
{
    return const_cast<LPWSTR>(StrStrNW(const_cast<LPCWSTR>(lpFirst), lpSrch, cchMax));
}
__inline LPSTR StrStrIA(__in LPSTR lpFirst, LPCSTR lpSrch)
{
    return const_cast<LPSTR>(StrStrIA(const_cast<LPCSTR>(lpFirst), lpSrch));
}
__inline LPWSTR StrStrIW(__in LPWSTR lpFirst, LPCWSTR lpSrch)
{
    return const_cast<LPWSTR>(StrStrIW(const_cast<LPCWSTR>(lpFirst), lpSrch));
}
__inline LPWSTR StrStrNIW(__in LPWSTR lpFirst, LPCWSTR lpSrch, UINT cchMax)
{
