...
void WppInitGlobalLogger(
        ...
        PUCHAR pLevel )
{
PWCHAR                     GRegValueName ; // L"WMI\\GlobalLogger\\d58c126f-b309-11d1-969e-0000f875a5bc" ;
...
HRESULT                    hr ;
   ...
   WppDebug(0,("WPP checking %S Logger",LoggerName));
   
   GregValueNameLength = sizeof(WCHAR) * (ULONG)(wcslen(DEFAULT_GLOBAL_LOGGER_KEY) + wcslen(LoggerName) + wcslen(L"\\")
                                          +  WPP_TEXTGUID_LEN ) + sizeof(UNICODE_NULL) ;
                                          
   //Allocate the blob to hold the data
   GRegValueName = ExAllocatePoolWithTag(WPP_POOLTYPE,GregValueNameLength,WPP_POOLTAG) ;

   if (!GRegValueName) {
      WppDebug(0,("Could not get memory for Registry Value Name %S",LoggerName));
      return;
   }
   ...
   if (wcscmp(LoggerName,L"GlobalLogger") == 0) {
        IsGlobalLogger = TRUE ;
#ifdef WPP_STRSAFE
        hr = StringCbPrintfW(GRegValueName,GregValueNameLength,L"%ws",DEFAULT_GLOBAL_LOGGER_KEY);
#else
        wcsncpy(GRegValueName,DEFAULT_GLOBAL_LOGGER_KEY,GregValueNameLength/sizeof(WCHAR));
        GRegValueName[(GregValueNameLength/sizeof(WCHAR))-1] = L'\0' ;
#endif
   } else {
#ifdef WPP_STRSAFE
        hr = StringCbPrintfW(GRegValueName,GregValueNameLength,L"%ws%ws\\",DEFAULT_LOGGER_KEY,LoggerName);
#else
        swprintf(GRegValueName,L"%ws%ws\\",DEFAULT_LOGGER_KEY,LoggerName);
        GRegValueName[(GregValueNameLength/sizeof(WCHAR))-1] = L'\0' ;
#endif 
   }
...
#ifdef WPP_STRSAFE   
   if FAILED(hr) {
       WppDebug(0,("WPP Failed to make Key name for %S ",LoggerName));
       ExFreePool(GRegValueName);
       return ;
   }
#endif 
   WppDebug(0,("WPP Made Key name %S for %S ",GRegValueName,LoggerName));
   //
   ...
   status = RtlQueryRegistryValues(RTL_REGISTRY_CONTROL | RTL_REGISTRY_OPTIONAL,
    ...
    if (!NT_SUCCESS(status) || Lstart == 0 ) {
        WppDebug(0,("WPP Auto Logger %S is not started (%S)",LoggerName,GRegValueName));
        ExFreePool(GRegValueName);  
        return ;
    }
   ...
   WppDebug(0,("WPP Querying for %S ",GRegValueName));
   ...
   status = RtlQueryRegistryValues(RTL_REGISTRY_CONTROL | RTL_REGISTRY_OPTIONAL,
                                   ...
                                   NULL,
                                   NULL);
   ExFreePool(GRegValueName);  // finished with this avoid having to include on all paths
           ...
           if (IsGlobalLogger) {
              *pLogger= WMI_GLOBAL_LOGGER_ID ;
              ...
              if ((status = WmiQueryTraceInformation(TraceHandleByNameClass,
                                                  ...
                                                  sizeof(TRACEHANDLE),
                                                  &ReturnLength,
                                                  (PVOID)&LoggerNameStr)) != STATUS_SUCCESS) {
                 *pLogger= (TRACEHANDLE)NULL;
                 WppDebug(0,("Could not get TraceHandle By name for %S, Status = 0x%08X",LoggerName,status));
              }
           }
           *pFlags = Lflags & 0x7FFFFFFF ;
           *pLevel = (UCHAR)(Llevel & 0xFF) ;
           if (*pLogger != (TRACEHANDLE)NULL) {
              WppDebug(0,("WPP Enabled via Global Logger Logger=%016I64X Flags=0x%08X Level=0x%02X",*pLogger,Lflags,Llevel));
           } else {
              WppDebug(0,("WPP Not enabled as Global Logger not active Flags=0x%08X Level=0x%02X",Lflags,Llevel));
           }

        }
   } else {
        WppDebug(0,("WPP Auto Logger %S No Flags/Levels Status=%08X",LoggerName,status));
   }                                  
}
    ...
    *Size = 0;
    ...
    switch(minorFunction)
    {
        case IRP_MN_REGINFO:
        {
            ...
            WppInitGlobalLogger(L"GlobalLogger",
#if defined(WPP_TRACE_W2K_COMPATABILITY)
                                regGuid,
#else
                ...
                // Check if Global logger is active            
                WppInitGlobalLogger(WPP_AUTOLOGGER,
#if defined(WPP_TRACE_W2K_COMPATABILITY)
                                regGuid,
#else
                                cb->Registration.ControlGuid,
