...
BOOL CModInfo::AddToInitString(CHAR* pszString)
{
	...
	ULONG dwLength = wcslen(m_pwszInitString) + wcslen(pwszNewString) + 1 + 2;
...
//////////////////////////////////////////////////////////////////////////
// CModInfo::GetInitPropsXMLOLEDB
//
//////////////////////////////////////////////////////////////////////////
BOOL CModInfo::GetInitPropsXMLOLEDB(ULONG*	pcPropSets, DBPROPSET**	prgPropSets)
{
	...
	CStorage* pCStorage = new CStorage; //IStream
	...
	*prgPropSets = NULL;
	...
	//See if we should handle this here
	if(!GetInitStringValue(L"XMLDS_STREAM", &pwszValue) )
		goto CLEANUP;

	if ( !GetInitStringValue(L"DATASOURCE", &pwszValue) )
		GetInitStringValue(L"DATA SOURCE", &pwszValue);
	if(!pwszValue)
		TESTC(FALSE);		

	pszXMLDoc = OpenXMLAsText(pwszValue);
	QCOMPARE(pCStorage, NULL);
	TEST2C_(hr = pCStorage->WriteAt(ulOffset, (void*)pszXMLDoc, /*cb*/strlen(pszXMLDoc), NULL), S_OK, S_FALSE);
	TEST2C_(hr = pCStorage->QueryInterface(IID_ISequentialStream, (void**) &pIStream), S_OK, S_FALSE);

	PropertyID		=	XMLPROP_DATASTREAM;
	SetProperty(PropertyID, DBPROPSET_XML_DBINIT, pcPropSets, prgPropSets, DBTYPE_IUNKNOWN, pIStream); 

	//Now process the xml:schema property if it exists
	PROVIDER_FREE(pwszValue);
	GetInitStringValue(L"XML:SCHEMA", &pwszValue);
	if(!pwszValue)
		goto CLEANUP;		//Schema not specified so we are done here

	VariantInit(&vVariant);
	V_VT(&vVariant) = VT_BSTR;
	V_BSTR(&vVariant) = SysAllocString(pwszValue);
	PropertyID		=	XMLPROP_XMLSCHEMA;
	SetProperty(PropertyID, DBPROPSET_XML_DBINIT, pcPropSets, prgPropSets, DBTYPE_VARIANT, &vVariant); 
	VariantClear(&vVariant);
