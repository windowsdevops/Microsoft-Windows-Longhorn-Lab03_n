...
BOOL CParseInitFile::Init(WCHAR* pwszFileName)
{
	...
	ConvertToMBCS(pwszFileName, m_pszFileName, wcslen(pwszFileName) + sizeof(CHAR));
	...
	)
{
	...
	ULONG ulNameLen = strlen(pszSectionName);
	HRESULT hr = S_OK;
	...
	while(hr == S_OK)
	{
		...
		if(pszStartofType)
		{
			...
			{
				ULONG ulNameLen2 = strlen(pszFirstReqSec);
	...
	while(hr==S_OK)
	{
		if(pszStart)
		{
			...
			if(pszEndData >= pszStart)
			{
				...
				ConvertToWCHAR(pszStart, m_rgURLs[eInterface], strlen(pszStart)+1);
			}
		}
	...
	while(hr==S_OK)
	{
		if(pszStart)
		{
			...
			if(pszEndData >= pszStart)
			{
				...
				ConvertToWCHAR(pszStart, m_rgQueryNames[lItem], strlen(pszStart)+1);
			}
		}
