...
CLEANUP:
	...
	while(pos)
	{
		...
		if(GetModInfo()->GetLocaleInfo())
		{
			...
			swprintf(strColName,L"%c%.2s%.4s", MapWCHAR(GetModInfo()->GetLocaleInfo()->MakeUnicodeChar(), LCMAP_UPPERCASE),
				pwszUnicodeInt,rCol.GetProviderTypeName());
		}
		else
		{						
			swprintf(strColName, L"C%02lu%.4s", udwColNum, rCol.GetProviderTypeName());
...
)
{
	...
	ULONG		cchColList=0;			// Keep track of ColList length
	DBORDINAL   iCol=0;
						...
						if (rCol.GetProviderType()==DBTYPE_DBTIMESTAMP)
						{
							...
							if (pwszDate)
							{
								...
								if ((wcslen(pwszDate)-2)>wScale)
								{
									...
									for (WORD i = 0;i<wScale;i++,pwszTrunc++);
...
)
{
	...
	// Argument checking
	hr = E_INVALIDARG;
	TESTC(eListType >= LT_COLNAME && eListType <= LT_LITERAL_SEARCH);
	TESTC(ppwszList != NULL);
	TESTC(pprgColumns == NULL || pcColumns != NULL);
	TESTC(eColsInList >= ALL_COLS_IN_LIST && eColsInList <= SEARCHABLE_AND_UPDATEABLE_COLS_IN_LIST); 
...
HRESULT CTable::GetQuotedName(WCHAR* pwszName, WCHAR** ppwszQuoted, BOOL fFromCatalog)
{
	...
	*ppwszQuoted = NULL;
	...
	ULONG cPrefixLen	= pPrefixLiteral->fSupported ? wcslen(pPrefixLiteral->pwszLiteralValue) : 0;
	ULONG cSuffixLen	= pSuffixLiteral->fSupported ? wcslen(pSuffixLiteral->pwszLiteralValue) : 0;
	ULONG cPeriodLen	= 1;
	...
	BOOL fFromCatalog)
{
	...
	*ppwszQualifiedName = NULL;
	...
	ULONG cSepLen		= pCatalogLiteral->fSupported ? wcslen(pCatalogLiteral->pwszLiteralValue) : 0;
	ULONG cPeriodLen	= 1;

	//Allocate enough room (max possible) for the resultant qualified string...
	ULONG cTotalLen = cSepLen + cPeriodLen + wcslen(pwszQuotedTable);
