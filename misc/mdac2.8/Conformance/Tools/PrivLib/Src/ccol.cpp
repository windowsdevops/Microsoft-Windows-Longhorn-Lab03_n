...
)
{
	...
	LONG_PTR	lPrecision=0;		// Precision
	LONG_PTR	lPrecUsed=0;		// Precision used 
	SHORT		sScale=0;			// Scale
	INT_PTR		iColLen=0;			// Length of the value seed
	INT_PTR		iLenLeft=0;			// Scale - length left in strDecimal to fill
	INT_PTR		ivalue=0;			// int value
	long		lvalue=0;			// long value
									// Also value returned from ReverseFind
	INT_PTR		lenwszData=0;		// length of wszData for first column
	...
	if (GetColNum() != ulIndexCol)
	{
		if ( lPrecUsed > sScale )
		{
			...
			while(wcslen(wszData) < (size_t)lPrecision)
			{
				iLenLeft = lPrecision - wcslen(wszData);
		...
		else
		{
			...
			for(ivalue=0; ivalue<sScale-lPrecUsed;ivalue++)
				wszData[2+ivalue] = L'0';

			wszData[sScale-lPrecUsed+2] = L'\0'; //+2 is for the decimal point char and leading zero

			while(wcslen(wszData+(sScale-lPrecUsed)+2) < (size_t)lPrecUsed)
			{
	...
	else
	{
		...
		if(lPrecision>=0 && lPrecision < lenwszData)
			wszData[lPrecision] = L'\0';
		...
		if (sScale > 0 && sScale <= lPrecision )
		{
			wszData[lenwszData] = L'.';
			wszData[lenwszData + 1] = L'\0';
			lenwszData = wcslen(wszData);

			for(ivalue=0;ivalue<sScale;ivalue++)// while(wcslen(wszData) < (ULONG)(sScale + 1 + lPrecision))
	...
	else if (sScale < 0)
	{
		...
		wszData[lPrecUsed+abs(sScale)] = L'\0';
	}
	...
	switch(wType) 
	{
		...
		case DBTYPE_I1:
			...
			if (wType == DBTYPE_UI1) 
				...
				swprintf(wszData,L"%i", -(ivalue % 0x7F));
			break;
		...
		case DBTYPE_CY:
		case DBTYPE_NUMERIC:
			if ((ulRowNum > 1) && (lPrecision < 11) && (lPrecUsed != sScale))
			{
				// If negative number
