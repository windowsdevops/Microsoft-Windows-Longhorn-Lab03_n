...
/////////////////////////////////////////////////////////////////////////////
HRESULT ConvertToMBCS(LPCWSTR pwsz, CHAR* psz, ULONG cStrLen, UINT CodePage)
{
	ASSERT(pwsz && psz);
...
CHAR* ConvertToMBCS(LPCWSTR pwsz, UINT CodePage)
{
	...
	ULONG cLen	= wcslen(pwsz) * sizeof(WCHAR);
	...
	WideCharToMultiByte(CodePage, 0, pwsz, -1, pszBuffer, cLen+1, NULL, NULL);
...
//		Previously allocated memory
/////////////////////////////////////////////////////////////////////////////
HRESULT ConvertToWCHAR(LPCSTR psz, WCHAR* pwsz, ULONG cStrLen)
{
	ASSERT(psz && pwsz);
	...
	*/
	...
	switch(dbDestType)
	{
		...
		case DBTYPE_VARNUMERIC:
		{
			...
			WCHAR	*pwszBuff = NULL;
			LONG	lScale = 0;
			ULONG	ul=0, cchIn=0, i=0, ulPrecision=0;
			LONG	lExp = 0;
			...
			for (usIdx = 0; usIdx < ((DB_VARNUMERIC *)pDestBuf)->precision; usIdx++)
				ASSERT( AddCharToVarNumericVal(wszValue[usIdx], (DB_VARNUMERIC *)pDestBuf, USHORT(cBytes-sizeof(DB_VARNUMERIC)+1)) );
		...
		{	
			...
			for (usIdx = 0; usIdx < ((DB_NUMERIC *)pDestBuf)->precision; usIdx++)
				ASSERT( AddCharToNumericVal(wszValue[usIdx], (DB_NUMERIC *)pDestBuf) );
...
// 
//----------------------------------------------------------------------
HRESULT DLL_EXPORT FreeAccessorBindings(
...
// 
//----------------------------------------------------------------------
HRESULT DLL_EXPORT FreeColAccess(
...
//
//-----------------------------------------------------------------------------
BYTE DLL_EXPORT bNumericPrecision(
	DBTYPE wType,	// @parm  [IN] type 
...
//---------------------------------------------------------------------
//
DLL_EXPORT  BOOL DefTestInterface(IDBInitialize* pIDBInitialize)
{
	TBEGIN
	HRESULT				hr;
...
//---------------------------------------------------------------------
//
DLL_EXPORT  BOOL DefTestInterface(IDBProperties* pIDBProperties)
{
	TBEGIN
	ULONG				cPropInfoSets = 0;
	ULONG				cPropSets = 0;
...
//---------------------------------------------------------------------
//
DLL_EXPORT  BOOL DefTestInterface(IDBCreateSession* pIDBCS)
{
	TBEGIN
	HRESULT				hr = E_FAIL;
...
//--------------------------------------------------------------------
//
DLL_EXPORT  BOOL DefTestInterface(IDBCreateCommand* pIDBCC)
{
	TBEGIN
...
//--------------------------------------------------------------------
//
DLL_EXPORT  BOOL DefTestInterface(IAccessor* pIAccessor)
{
	TBEGIN
...
//
////////////////////////////////////////////////////////////////////////////
DLL_EXPORT	BOOL	VerifyRefCounts
(	
...
HRESULT	ReplaceString(WCHAR* pwszBuffer, WCHAR* pwszTokens, WCHAR* pwszReplace)
{
	...
	ULONG ulTokenLen = wcslen(pwszTokens);
	ULONG ulReplaceLen = wcslen(pwszReplace);
...
)
{
	...
	cchWritten = MultiByteToWideChar(CP_ACP, 0,  szBufUpper, -1, pwszSource, wcslen(pwszSource)+1);
	ASSERT(cchWritten > 0);
...
WCHAR * MakeObjectName(WCHAR * pwszModuleName, ULONG ulMaxNameLen)
{
