...
)
{
	...
	if (fQI)
	{
		ASSERT(VerifyInterface(pIUnknown, IID_IDBSchemaRowset,
...
HRESULT CConstraints::CheckDropConstraint(DBCONSTRAINTDESC *pConsDesc)
{
	...
	DBCOUNTITEM			cConstraints = m_cConstraints;
	DBCONSTRAINTDESC	*rgConstraints = m_rgConstraints;
	DBCOUNTITEM			cConstraints2 = 0;
	DBCONSTRAINTDESC	*rgConstraints2 = NULL;
	CWString			wszTableCatalog(m_pwszTableCatalog);
	CWString			wszTableSchema(m_pwszTableSchema);
	CWString			wszTableName(m_pwszTableName);
	CWString			wszConsCatalog(m_pwszConsCatalog);
	CWString			wszConsSchema(m_pwszConsSchema);
	CWString			wszConsName(m_pwszConsName);
	...
	// refresh constraints array
	TESTC_(GetConstraints(
			(LPWSTR)(LPCWSTR)wszTableCatalog, 
			(LPWSTR)(LPCWSTR)wszTableSchema, 
			(LPWSTR)(LPCWSTR)wszTableName,
			(LPWSTR)(LPCWSTR)wszConsCatalog, 
			(LPWSTR)(LPCWSTR)wszConsSchema, 
			(LPWSTR)(LPCWSTR)wszConsName), S_OK);

	// compare the 2 lists
	TESTC(cConstraints + 1 == m_cConstraints);
	TESTC_(CheckIncludedConstraints(1, pConsDesc), E_FAIL);
	
	cConstraints2	= m_cConstraints;
	rgConstraints2	= m_rgConstraints;
	m_cConstraints	= cConstraints;
	m_rgConstraints	= rgConstraints;

	// check the constraints
	if (!CHECK(CheckIncludedConstraints(cConstraints2, rgConstraints2), S_OK))
	{
		m_cConstraints	= cConstraints2;
		m_rgConstraints	= rgConstraints2;
		goto CLEANUP;
	}
