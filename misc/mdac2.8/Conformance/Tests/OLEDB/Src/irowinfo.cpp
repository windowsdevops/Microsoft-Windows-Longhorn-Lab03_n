...
{	
	...
	do 
	{
		...
		// to figure out where CreateRowsetObject left it).  It
...
)
{
	...
	for (ULONG uTestIndex=0; uTestIndex < cNumRequiredProperties; uTestIndex++) 
	{
...
int TCGetPropParam::Variation_5()
{
	...
	// Cleanup is necessary following a failure from this point onward...
	if(!CHECK(GetProperties(2,rgProperties,&uRsltPropCnt,&rgRsltProps), DB_S_ERRORSOCCURRED))
		goto CLEANUP;
