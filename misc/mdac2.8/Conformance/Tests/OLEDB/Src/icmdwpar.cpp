...
ULONG FormatString(WCHAR ** ppwszDest, WCHAR * pwszFmt, ULONG cArgs, ...);
ULONG FormatStringFromArray(WCHAR ** ppwszDest, WCHAR * pwszFmt, ULONG cArgs, WCHAR ** ppInsertArray);
BOOL AddParam(
	DB_UPARAMS iParam,
	DBORDINAL iCol,
	DBPARAMIO eParamIO,
...
CLEANUP:
	...
	WCHAR * BuildTokenString(enum TOKEN_ENUM eToken, DBORDINAL * pcColsAdded, DB_LORDINAL ** prgColsAdded)
	{
		...
		const ulColNameMax = m_ulMaxParamName;
		...
		if (m_ulCreateFlags & CREATE_LONG_NAMES || m_ulCreateFlags & CREATE_MAX_NAMES)
		{
			...
			if (GetModInfo()->GetLocaleInfo())
			{
				...
				if (!GetModInfo()->GetLocaleInfo()->MakeUnicodeIntlString(pwszParamName+wcslen(pwszParamName), 
					ulMaxParamName-wcslen(pwszParamName)+1))
					goto CLEANUP;
			}
			else
			{
				for (ULONG ichar=wcslen(pwszParamName); ichar < ulMaxParamName; ichar++)
	...
	)
{
	...
	*((DBLENGTH *)((BYTE *)pData + rgDbBindInfo->obLength)) = sizeof (WCHAR) *10;
...
ULONG FormatStringFromArray(WCHAR ** ppwszDest, WCHAR * pwszFmt, ULONG cArgs, WCHAR ** ppInsertArray)
{
	ULONG ulStringLen = 0, cch=0, cInserts=0, iIns, ulInsNo;
	WCHAR * pwszIns = NULL;
		...
		for (iParamRow=0; iParamRow < cParamSets; iParamRow++)
		{
			...
			for (iParam=0; iParam < cParams; iParam++)
			{
				...
				if ((*ppDBBINDINFO)[iParam].eParamIO == DBPARAMIO_OUTPUT)
				{
					...
					STATUS_BINDING((*ppDBBINDINFO)[iParam], pData) = 99; 
					LENGTH_BINDING((*ppDBBINDINFO)[iParam], pData) = LONG_MAX-5; 
				}
...
CICmdWParams::StorePassByRefPointers(DB_UPARAMS cRows, BYTE * pData)
{
	...
	if (cRows > 1)
	{
		ASSERT("Function to be updated for multiple parameter sets");
	}
...
private:
	...
	ICmdWParSequentialStream * m_pISeqStream;
...
//	WCHAR * pwszProcNameRoot=L"\"''Stra# _* & % n  \"";
	...
	pwszRootName = MakeObjectName(pwszProcNameRoot, pTableLiteral->cchMaxLen);
...
int TCCommandExecute_Rowset::Variation_4()
{
	...
	if (m_cBindings < 3)
	{
		odtLog << L"Too less columns in this table for this variation\n";
		goto CLEANUP;
	}
...
int TCCommandExecute_Rowset::Variation_39()
{
	...
	for( count=1; count <= cColumns; count++)
	{
		...
		if (NewCol.GetProviderType() == DBTYPE_BYTES && NewCol.GetIsLong())
	...
	if (ListPosition)
	{
		...
		while (ListPosition != NULL)
		{
			DBTypeList.InsertAfter(HeadPosition, DBBlobList.GetNext(ListPosition));
		}
	}
	// We have constructed the list.  Now create the table.
...
CLEANUP:
	...
	ReleaseInputBindingsForArray(cNumRows, cbRowSize, m_cBindings, m_rgBindings, (BYTE *)pData);
...
{ 
	...
	//This sets it to one row of data.
	if (!CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid, DBACCESSOR_PARAMETERDATA,
				m_cBindings, m_rgBindings, &m_pData, ulRowNum = NextInsertRowNum(), m_cParamColMap,
				(DB_LORDINAL *)m_rgParamColMap), S_OK))
	{
		return FALSE;
	}
	...
	// Now execute ICommand.
	TESTC_(m_pICommand->Execute(NULL, IID_NULL,
			&m_DbParamsAll, &cRowsAffected, NULL), S_OK);
...
{ 
	...
	TESTC_(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid, DBACCESSOR_PARAMETERDATA,
		m_cBindings, m_rgBindings, &m_pData, ulRowNum = NextInsertRowNum(), m_cParamColMap, (DB_LORDINAL *)m_rgParamColMap), S_OK);
...
{ 
	...
	for (iParam = 0; iParam < cParams; iParam++)
	{
		...
		{
			STATUS_BINDING(pBINDING64[iParam], pData64) = 99;
			LENGTH_BINDING(pBINDING64[iParam], pData64) = 99;
...
LPVOID TCIUnknown::MakeData(CCol ColInfo, ULONG * pcbDataSize, ULONG ulSeed, enum EVALUE eValue)
{
	...
	switch(wType)
	{
		...
		case DBTYPE_VARNUMERIC:
			...
			break;
		case DBTYPE_STR:
//			cbSrcData = ColInfo.GetColumnSize();
			cbSrcData = strlen((CHAR *)pvData);
			break;
		case DBTYPE_WSTR:
			cbSrcData = wcslen(pwszDataItem)*sizeof(WCHAR);
			break;
		default:
...
TESTRESULT TCIUnknown::TestActiveError(enum ACTIVE_ERROR eTest, REFIID iidParamObject)
{
	...
	for (iCol=1; iCol <= m_pTable->CountColumnsOnTable(); iCol++)
	{
		...
		ABORT_COMPARE(FAILED(m_hr), TRUE);
		...
		if (((m_hr == DB_E_ERRORSOCCURRED && ulInputStatus != DBSTATUS_S_OK) || m_hr != DB_E_ERRORSOCCURRED)
			&& !COMPARE(cRef, 1))
		{
...
//	cbParamRowSize = ROUND_UP(cbParamRowSize, ROUND_UP_AMOUNT);
	...
	if (!SUCCEEDED(CreateStorageObject(iidVerifyObject, (IUnknown **)&m_pISeqStream)))
	{
