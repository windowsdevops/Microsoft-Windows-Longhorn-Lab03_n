...
public:
	...
	virtual IsOptionalCP(REFIID riid) {return IID_IDBAsynchNotify==riid;};
...
public:
	...
	virtual BOOL CreateSource();
	virtual BOOL CauseNotification();
	virtual IsOptionalCP(REFIID riid) {return IID_IRowsetNotify==riid || IID_IDBAsynchNotify==riid;};
...
BOOL CTestNotify::Terminate()
{
	...
	for(ULONG i=0; i<m_vectSources.GetCount(); i++) 
...
BOOL CTestNotify::VerifyConnectionPoints(ULONG cConnectionPoints, IConnectionPoint** rgpIConnectionPoints)
{
	...
	for(i=0; i<cConnectionPoints; i++)
	{
		...
		//We need to make sure the enum contains IRowsetNotify (and optionally IDBAsynchStatus)
		if(VerifyEqualInterface(rgpIConnectionPoints[i], pCSource(0)->GetCP(i)))
		{
			IID iid;
...
int TCGetConnectionInterface::Variation_3()
{
	...
	for(i=0; i<3; i++) 
	{
		...
		//Test returned pICP with our CP, (which is a IRowsetNotifyCP or IDBAsynchNotify)
		TESTC(VerifyEqualInterface(pICP, pCSource(i)->GetCP()));
...
int TCGetConnectionPointContainer::Variation_2()
{
	...
	for(ULONG i=0; i<3; i++) 
	{
		...
		TESTC(VerifyEqualInterface(pICPC, pCSource(i)->pICPC()));
		...
		while(cFetched)
		{
			...
			TESTC(VerifyEqualInterface(pICPC, pICPC2));
...
int TCFindConnectionPoint::Variation_3()
{
	...
	{	
		...
		TESTC(VerifyEqualInterface(rgpICP[i], pCSource(0)->GetCP()));
	}

	//Verify Reference count
	TESTC(VerifyRefCounts(GetRefCount(pCSource(0)->GetCP()), cRefCount+10));	

	//Now Release 2 and verify
	SAFE_RELEASE(rgpICP[0]);
	SAFE_RELEASE(rgpICP[1]);
	TESTC(VerifyRefCounts(GetRefCount(pCSource(0)->GetCP()), cRefCount+8));
...
int TCFindConnectionPoint::Variation_6()
{
	...
	for(ULONG i=0; SupportedCP[i]; i++)
	{
		...
		TESTC(VerifyEqualInterface(pICP, pCSource(0)->GetCP()));
		SAFE_RELEASE(pICP);
	}
