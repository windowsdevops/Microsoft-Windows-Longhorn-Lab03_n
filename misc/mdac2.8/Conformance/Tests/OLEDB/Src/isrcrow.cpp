...
BOOL CSourcesRowset::ReadParseName(const CLSID clsid)
{
	...
	// enumerators.
	while(	(S_OK == m_pIRowset->GetNextRows(NULL,0,1,&cRow,&pHRow))
...
BOOL CSourcesRowset::VerifyParseDisplayName_root(IParseDisplayName	* pIParseDisplayName)
{
	...
	for(ULONG i=0; i<(g_cOLEDBProvNames+g_cOLEDBProvMDPNames); i++)
	{
		// Get a moniker
...
ULONG CSourcesRowset::GetNextRowsWithoutRelAllRows()
{
	...
	while((m_hr=pIRowset->GetNextRows(NULL,0,FETCH_ROW_STD,&cRow,&pHRow)) == S_OK)
	{
		// verify that the proper return value is generated when the rows are not released
		TESTC(cRow == FETCH_ROW_STD);
		m_hr=pIRowset->GetNextRows(NULL,0,FETCH_ROW_STD, &cRow,&pHRow);
		if (SUCCEEDED(m_hr))
		{
		...
		// release the rows and go for the next chunk of rows
		TESTC_(pIRowset->ReleaseRows(FETCH_ROW_STD, hRow, NULL, 0, NULL), S_OK);
	}
...
int TCSourcesRowset_RootEnumerator::Variation_1()
{
	...
	// Pass a NULL pointer 
	if (CHECK(m_hr=m_pISrcRow->QueryInterface(IID_IParseDisplayName,NULL), E_INVALIDARG))
		fResult = TEST_PASS;
...
BOOL TCSourcesRowset_ProviderEnumerator::Init()
{
	...
	if (!fProviderFound)
	{
		...
		{
				//look for the provider's enumerator in key read from registry now that test knows which provider to run against
				//if (!wcsicmp(g_rgwszSourceName[i] ,wszEnum) )
				if	(	
...
{ 
	...
	for (nIndex=0; nIndex<nThreads; nIndex++)
	{
		CloseHandle(hThread[nIndex]);
		CHECK(m_rgResult[nIndex], S_OK);
		if (!COMPARE(m_rgRowsNo[nIndex] == cRefRowsNo, TRUE))
