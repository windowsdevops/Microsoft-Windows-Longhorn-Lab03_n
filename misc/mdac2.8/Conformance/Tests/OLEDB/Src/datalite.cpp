...
)
{
	...
	if ( m_hr == S_OK )
	{
		...
		ULONG cchMakeData = wcslen(wszData);
	...
	*(DBSTATUS *)(m_pExpectedData+offsetof(DATA, sStatus)) = dbsStatus;
	...
	if ( FAILED(m_hr))
	{
		...
		if( !COMPARE(m_pIConvertType->CanConvert
								(
									(fIsVariant ? DBTYPE_VARIANT : wColType), 
									wType | dbByRefMask,
...
BOOL TCDATALITE::VerifyResult(DBTYPE wDstType)
{
	...
	case DBTYPE_R8:
		...
		dblTolerance = 16.0/pow(2,53); // large tolerance in case we didn't create the data
	...
	case DBTYPE_CY:
	{
		...
		if( wSrcType == DBTYPE_R4 )
			dblTolerance = 1.0/pow(10,3);  // 3 = 7(real precision) - 4 (CY scale by 10,000)
		else if( wSrcType == DBTYPE_R8 )
			dblTolerance = 1.0/pow(10,9);
		else 
			dblTolerance = 0.0;
