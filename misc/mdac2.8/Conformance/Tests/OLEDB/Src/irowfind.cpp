...
class TCIRowsetFind : public CRowsetObject
{
	...
	protected:
		...
		(
			...
			HRESULT			hrExpected,			// Expected HRESULT
...
)
{
	...
	if (m_pFindValue && 
		DBSTATUS_S_OK == *(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus)))
	{
		switch ( wdbType )
		{
		case DBTYPE_BSTR:
			SysFreeString( *(BSTR *)(m_pFindValue+offsetof(FindValueInfo, pValue)) );
			break;
		case DBTYPE_VARIANT:
			VariantClear((VARIANT *)(m_pFindValue+offsetof(FindValueInfo, pValue)));
			break;
		case DBTYPE_STR | DBTYPE_BYREF:
		case DBTYPE_WSTR | DBTYPE_BYREF:
		case DBTYPE_VARIANT | DBTYPE_BYREF:
			PROVIDER_FREE(*(void **)(m_pFindValue+offsetof(FindValueInfo, pValue)));
			break;
		case DBTYPE_BSTR | DBTYPE_BYREF:
		{
			BSTR *pbyrefBstr = *(BSTR **)(m_pFindValue+offsetof(FindValueInfo, pValue));
			SysFreeString( *pbyrefBstr );
			PROVIDER_FREE(pbyrefBstr);
			break;
		}	
		case DBTYPE_IUNKNOWN:
	...
	if(m_pISeqStream)
	{
		delete m_pISeqStream;
		m_pISeqStream = NULL;
	}
	else
	{
		PROVIDER_FREE(m_pFindValue);
		m_pFindValue = NULL;
	}
...
)
{
	...
	HROW *		rghRows = ( phRows ? phRows : NULL );
	...
	//setup
	CreateFindValueAccessor(CompareOp, pCTable, ulRowToMatch, ulColToMatch, wColType, eSubCompare, fUseStream, &wNewCompareOp);
	...
	if ( !COMPARE(ulRowsExpected,cRowsObtained) )
	{
		fTestPass = TEST_FAIL;
		goto CLEANUP;
	}

	// Verify row positions of rows in rghRows.
	if ( fCheckRows )
	{
		for ( ULONG i=0; i < ulRowsExpected; i ++ )
		{
			fTestPass = VerifyRowPosition(rghRows[i], 
	...
	// Cleanup
CLEANUP:
	if ( fReleaseRows && cRowsObtained > 0 )
	{
	...
	case DBCOMPAREOPS_CONTAINS:
		{
			...
			case DBTYPE_NULL:
				return FALSE;
			default:
				odtLog << L"Unexpected Type for CONTAINS" << ENDL;
				return FALSE;
			}
		break;
		}
	...
	case DBCOMPAREOPS_NOTCONTAINS:
		{
			...
			case DBTYPE_NULL:
				return FALSE;
			default:
				odtLog << L"Unexpected Type for NOTCONTAINS or NOTBEGINSWITH" << ENDL;
				return FALSE;
			}
		break;
		}
	...
	case DBCOMPAREOPS_GT:
		switch ( wColType )
		{
			...
			case DBTYPE_NULL:
				return FALSE;
			default:
				odtLog << L"Unexpected Type for GT/GE" << ENDL;
				return FALSE;
			}
		break;		
	case DBCOMPAREOPS_LT:
	case DBCOMPAREOPS_LE:
		switch ( wColType )
		{
			...
			case DBTYPE_NULL:
				return FALSE;
			default:
				odtLog << L"Unexpected Type for LT/LE" << ENDL;
				return FALSE;
		}
		break;
	case DBCOMPAREOPS_NE:
...
)
{
	...
	case DBCOMPAREOPS_NOTBEGINSWITH:
		...
		default:
			odtLog << L"Unexpected Type for NOTBEGINSWITH/NOTCONTAINS" << ENDL;
			return FALSE;
		}
		break;
	...
	case DBCOMPAREOPS_BEGINSWITH:
		...
		default:
			odtLog << L"Unexpected Type for BEGINSWITH" << ENDL;
			return FALSE;
		}
		break;
	case DBCOMPAREOPS_CONTAINS:
		...
		case SUBOP_CONTAINS_END:
		{
			...
			case DBTYPE_WSTR:
				...
				*pcbDataLength = cbDataLength - (cchTrim * sizeof(WCHAR));
				break;
			case DBTYPE_STR:
				{
				...
				*pcbDataLength = cbDataLength - cbTrim;
				}
...
)
{
	...
	TESTC(CreateCoerceFindValueAccessor(DBCOMPAREOPS_EQ, g_pCTable, 1, ulColToFind, wBindingType));

	fTestPass = TEST_FAIL;

	TESTC_(m_hr = m_pIRowsetFind->FindNextRow(DB_NULL_HCHAPTER, m_hRowsetFindAccessor, 
			 ...
			 fSubTestPass = 
				CallFindNextRows
					(	
						...
						(ulRowIndex <= ulRowCount ? S_OK : DB_S_ENDOFROWSET),	// HRESULT to verify
						(ulRowIndex <= ulRowCount ? 1 : 0),						// How many rows to expect.
						TRUE,						// Release Rows
				...
				{
					fTestPass = TEST_FAIL;
					odtLog<<"Error at ColIndex "<<ulColIndex<<" and at RowIndex "<<ulRowIndex<< ENDL;
					odtLog<<"-------------------------------------------------------------"<< ENDL;
					break;
				}
			}
...
BOOL	TCIRowsetFind::IStorageCompareTest
(
	CTable *		pCTable,
	DBCOMPAREOP		CompareOp,
	eSUBCOMPAREOP	eSubCompare
)
{
	...
	{			
		...
		// autoinc column that we don't know the value of.
		if(	IsColumnMinimumFindable(&TempCol, CompareOp) &&
			(TempCol.GetProviderType() == DBTYPE_STR || TempCol.GetProviderType() == DBTYPE_BYTES) &&
			TempCol.GetIsLong())
		{
			for ( ULONG ulRowIndex = 1; ulRowIndex <= ulRowCount; ulRowIndex++ )
			{
...
BOOL TCIRowsetFind::IsColumnMinimumFindable(CCol *pCol, DBCOMPAREOP CompareOp)
{
	...
	if (pCol->GetProviderType() == DBTYPE_VARIANT)
		wTargetType = pCol->GetSubType();
	else
		wTargetType = pCol->GetProviderType();

	if ( wTargetType == DBTYPE_EMPTY || wTargetType == DBTYPE_NULL )
		return FALSE;

	if (IsStringCompareOp(CompareOp) && !IsStringType(wTargetType))
		return FALSE;

	if ((wTargetType == DBTYPE_BOOL ||
		wTargetType == DBTYPE_ERROR ||
		wTargetType == DBTYPE_GUID ||
		wTargetType == DBTYPE_BYTES ) &&
...
BEG_TEST_CASE(NULL_Collation, TCIRowsetFind, L"Test Null collatioans")
...
public:
	...
	// {{ TCW_TESTVARS()
	// @cmember Delete last row. *pBookMark=DBBMK_LAST. DB_E_BADBOOKMARK
	int Variation_1();
	// @cmember Delete 4th row.  *pBookMark = 2, cRows=1. Match on what was in 4th row.  S_OK and two hrows.
	int Variation_2();
	// @cmember Delete 1st row.  *pBookMark=4, cRows=-1.  Match on what was in 1st row.  DB_S_ENDOFROWSET and no rows.
	int Variation_3();
...
BEG_TEST_CASE(Deleted_Rows, TCIRowsetFind, L"Test with deleted rows")
	TEST_VARIATION(1, 		L"Delete last row. *pBookMark=DBBMK_LAST. DB_E_BADBOOKMARK")
	TEST_VARIATION(2, 		L"Delete 4th row.  *pBookMark = 2, cRows=1. Match on what was in 4th row.  S_OK and two hrows.")
	TEST_VARIATION(3, 		L"Delete 1st row.  *pBookMark=4, cRows=-1.  Match on what was in 1st row.  DB_S_ENDOFROWSET and no rows.")
...
public:
	...
	int Variation_2();
	// @cmember Delete 4th row. pBookmark=3rd row,cRows=2. Match 3rd row.  Verify S_OK and 3,5 hrows
	int Variation_3();
	// @cmember Delete last row.  pBookmark=DBBMK_LAST,cRows=1. DBCOMPAREOPS_IGNORE.  Verify 4th row returned.
	int Variation_4();
...
BEG_TEST_CASE(RemoveDeleted, TCIRowsetFind, L"Test in context of RemoveDeleted")
	...
	TEST_VARIATION(3, 		L"Delete 4th row. pBookmark=3rd row,cRows=2. Match 3rd row.  Verify S_OK and 3,5 hrows")
	TEST_VARIATION(4, 		L"Delete last row.  pBookmark=DBBMK_LAST,cRows=1. DBCOMPAREOPS_IGNORE.  Verify 4th row returned.")
	TEST_VARIATION(5, 		L"pBookmark=NULL, cRows = 2. match 3rd.  Delete 2nd and 5th rows. Find with cRows= -1 and match third")
END_TEST_CASE()
#undef THE_CLASS
// }} TCW_TESTCASE_END
// }} TCW_TEST_CASE_MAP_END
...
int Boundary::Variation_1()
{
	...
	//setup
	CreateFindValueAccessor(DBCOMPAREOPS_EQ, g_pCTable, 1, g_ulColNum, g_wColType, SUBOP_EMPTY);
...
int Boundary::Variation_7()
{
	...
	for (ulColIndex = 1; ulColIndex <= ulColNum; ulColIndex++ )
		{			
			g_pCTable->GetColInfo(ulColIndex, TempCol);
			for ( DWORD CompareOp = DBCOMPAREOPS_EQ; CompareOp <= DBCOMPAREOPS_IGNORE; CompareOp++ )
			{
				if ( !ValidateCompareOp ( TC_FindCompareOps(TempCol.GetColID()), CompareOp ) &&
					IsColumnMinimumFindable(&TempCol, CompareOp) )
				{
					fTestPass = 
						CallFindNextRows(	g_pCTable,	// CTable pointer
...
int Boundary::Variation_8()
{
	...
	// Lets create the accessor.
	if ( FAILED( m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 0, NULL, sizeof(DBLENGTH)+sizeof(DBSTATUS), 
						&m_hRowsetFindAccessor, NULL)) )
	{
		fTestPass = TEST_FAIL;
		goto CLEANUP;
...
// {{ TCW_VAR_PROTOTYPE(11)
//*-----------------------------------------------------------------------
// @mfunc DB_E_CANTSCROLLBACKWARDS.  cRows > 1
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
int Boundary::Variation_20()
{
	...
	//setup
	CreateFindValueAccessor(DBCOMPAREOPS_EQ, g_pCTable, 1, ulColToFind, wColType, SUBOP_EMPTY);

	// Overwrite the status to search for NULL
	ASSERT(DBSTATUS_S_OK == *(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus))); 
	*(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus)) = DBSTATUS_S_ISNULL;
...
int OutputRowHandleAllocation::Variation_2()
{
	...
	CreateFindValueAccessor(DBCOMPAREOPS_EQ, g_pCTable, 0, g_ulColNum, g_wColType, SUBOP_EMPTY);
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_EQ, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_BEGINSWITH, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_GT, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_EQ, SUBOP_ALWAYS_EQ);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_GE, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_LT, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_LE, SUBOP_ALWAYS_EQ);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_LE, SUBOP_EMPTY);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_CONTAINS, SUBOP_CONTAINS_BEGIN);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_CONTAINS, SUBOP_CONTAINS_MIDDLE);
}
// }}
...
{
	return IStorageCompareTest(g_pCTable, DBCOMPAREOPS_CONTAINS, SUBOP_CONTAINS_END);
}
// }}
...
int CaseSensitive_Compares::Variation_15()
{
	...
	CreateFindValueAccessor(DBCOMPAREOPS_BEGINSWITH | DBCOMPAREOPS_CASEINSENSITIVE, g_pCTable, 1, ulColToFind, wColType, SUBOP_EMPTY);
...
int CaseSensitive_Compares::Variation_18()
{
	...
	//setup
	//set up a INSENSITIVE match binding
	CreateFindValueAccessor(DBCOMPAREOPS_CONTAINS | DBCOMPAREOPS_CASEINSENSITIVE, g_pCTable, 1, ulColToFind, wColType, SUBOP_CONTAINS_BEGIN);
...
{	
	...
	for (ulColIndex=1; ulColIndex <= g_pCTable->CountColumnsOnTable(); ulColIndex++)
	{
		...
		if(m_hr == S_OK)
		{
			...
			CreateFindValueAccessor(DBCOMPAREOPS_NE, g_pCTable, 0, ulColIndex, 
			...
			if(m_hr == S_OK)
			{
				...
				CHECK(pIRowsetIden->IsSameRow(phrowNULL[0], phrowNOTNULL[0]), S_FALSE);			
...
int TCCompareOps_Ignore::Variation_1()
{
	...
	// match 4th row and fetch 2
	// new fetch position should be after 5th
	fTestPass = CallFindNextRows(		
									g_pCTable,		// Table to find from
									NULL,				// Bookmark to fetch from, if any
									0,					// Length of bookmark
	...
	if (!COMPARE(VerifyRowPosition(phrow[0], 8, g_pCTable), TRUE))
		goto CLEANUP;
...
int TCCompareOps_Ignore::Variation_3()
{
	...
	fTestPass = CallFindNextRows(		
									...
									g_ulColNum,		// Column to match
									g_lRowLast,		// Is there a row where the find should happen? 0 - no match
									DB_S_ENDOFROWSET,				// Expected HRESULT
									0,					// Expected count of rows
									TRUE,				// flag to Release rows (optional)
									DBCOMPAREOPS_IGNORE,	   // Any particular preference for comparing? (optional)
									SUBOP_EMPTY					// Some comparisions are rich enough to deserve a mulitple comparision operations
...
int AccessorTests::Variation_1()
{
	...
	pMakeData = (BYTE *)WSTR2DBTYPE(wszData, wColType, &usize );
	cbDataLength = GetDataLength(pMakeData, wColType, usize);
	...
	// Lets create the accessor.
	if ( FAILED( m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 1, &Binding, cbDataLength, 
					&m_hRowsetFindAccessor, NULL)) )
			goto CLEANUP;

	m_pFindValue = (BYTE *)PROVIDER_ALLOC(cbDataLength);
	memcpy(m_pFindValue+Binding.obValue, pMakeData, cbDataLength);
...
// {{ TCW_VAR_PROTOTYPE(1)
//*-----------------------------------------------------------------------
// @mfunc Delete last row. *pBookMark=DBBMK_LAST. DB_E_BADBOOKMARK
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
// {{ TCW_VAR_PROTOTYPE(2)
//*-----------------------------------------------------------------------
// @mfunc Delete 4th row.  *pBookMark = 2, cRows=1. Match on what was in 4th row.  S_OK and two hrows.
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
// {{ TCW_VAR_PROTOTYPE(3)
//*-----------------------------------------------------------------------
// @mfunc Delete 1st row.  *pBookMark=4, cRows=-1.  Match on what was in 1st row.  DB_S_ENDOFROWSET and no rows.
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
// {{ TCW_VAR_PROTOTYPE(5)
//*-----------------------------------------------------------------------
// @mfunc pBookmark=NULL, cRows=-2, match 3rd row. Delete 3rd. Match 2nd
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
// {{ TCW_VAR_PROTOTYPE(3)
//*-----------------------------------------------------------------------
// @mfunc Delete 4th row. pBookmark=3rd row,cRows=2. Match 3rd row.  Verify S_OK and 3,5 hrows
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
// {{ TCW_VAR_PROTOTYPE(4)
//*-----------------------------------------------------------------------
// @mfunc Delete last row.  pBookmark=DBBMK_LAST,cRows=1. DBCOMPAREOPS_IGNORE.  Verify 4th row returned.
//
// @rdesc TEST_PASS or TEST_FAIL
//
int RemoveDeleted::Variation_4()
{
	...
	if (!COMPARE(VerifyRowPosition(phRows[0], g_lRowLast-1, g_pCTable), TRUE))
		fTestPass = TEST_FAIL;
...
// {{ TCW_VAR_PROTOTYPE(5)
//*-----------------------------------------------------------------------
// @mfunc pBookmark=NULL, cRows = 2. match 3rd.  Delete 2nd and 5th rows. Find with cRows= -1 and match third
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
{ 
	...
	//create the table
	pTable = new CTable(m_pThisTestModule->m_pIUnknown2, (WCHAR *)g_wszModuleName, USENULLS);
	...
	for (ulColIndex=1; ulColIndex <= pTable->CountColumnsOnTable(); ulColIndex++)
	{
		...
		if((DBTYPE_STR == wTargetType ||
		   ...
		   &&
			TempCol.GetUpdateable())
		{
			if(!ValidateCompareOp(TC_FindCompareOps(TempCol.GetColID()), DBCOMPAREOPS_CONTAINS | DBCOMPAREOPS_CASEINSENSITIVE))
				continue;

			//set up a INSENSITIVE match binding
			CreateFindValueAccessor(DBCOMPAREOPS_CONTAINS | DBCOMPAREOPS_CASEINSENSITIVE, pTable, cRowsOnTable+1, 
							ulColIndex,	wTargetType, SUBOP_CONTAINS_BEGIN);
			if (DBSTATUS_S_ISNULL == *(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus)))
			{
				ReleaseFindValueAccessor(wTargetType);
				continue;
			}				

			m_hr = m_pIRowsetFind->FindNextRow(DB_NULL_HCHAPTER, m_hRowsetFindAccessor, m_pFindValue, 
												DBCOMPAREOPS_CONTAINS | DBCOMPAREOPS_CASEINSENSITIVE, 0, NULL, 0, 
												1, &cRowsObtained, &phrow);
			// Verify HRESULT
			if (m_hr == DB_E_BADCOMPAREOP)
			{
			...
			{
				odtLog << "Expected a match, but none found!" << ENDL;
				odtLog << "Error on colIndex: " << ulColIndex << "." << ENDL;
				goto CLEANUP;
			}
			...
			TESTC_(m_hr, S_OK);
			TESTC(cRowsObtained == 1);
			TESTC(VerifyRowPosition(*phrow, cRowsOnTable+1, pTable));
		}
	}
