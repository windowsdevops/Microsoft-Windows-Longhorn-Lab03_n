...
static fProperRangeForFillFactor = TRUE;
...
//-------------------------------------------------------------------------
WCHAR* TCIIndexDefinition::BuildValidName(ULONG length, WCHAR* pattern)
{
	WCHAR*	pwszBuffer;
...
// the pattern is supposed to be shorter than the string to be build
//-------------------------------------------------------------------------
WCHAR* TCIIndexDefinition::BuildInvalidName(ULONG length, WCHAR* pattern, WCHAR* invchars)
{
	WCHAR*	pwszBuffer;
...
{ 
	...
	TESTC_(SetProperty(&pCD->rgPropertySets, &pCD->cPropertySets, DBPROP_COL_AUTOINCREMENT, 
...
{ 
	...
	COMPARE(m_pCIIndexDefinition->DropIndex(&pTable->GetTableID(), (DBID*)IndexID), TRUE);
