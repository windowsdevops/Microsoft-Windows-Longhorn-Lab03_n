	...
	)
{
	...
	*phAccessor = DB_NULL_HACCESSOR;
	FreeAccessorBindings(*pcBindings, *prgBindings);
	*pcBindings = 0;
	*prgBindings = NULL;
	if(bReleaseIAcc)
...
TESTRESULT CQuickTest::testAddColumn()
{
	...
	{
		//Its an old column.
		if (CompareDBID(rgColDesc1[cOldCol].dbcid, 
...
TESTRESULT CQuickTest::testDropTable()
{
	...
	//Create a table.
	TESTC_(hr = pTable->CreateTable(0, 0, NULL), S_OK);
