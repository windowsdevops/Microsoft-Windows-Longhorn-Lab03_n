...
public:
	//constructors
	CConnectInfo(LPCWSTR pwszInitString)
	{
...
public:
	...
	void		Remove(POSITION pos)
	{
		if(pos)
		{
			CConnectInfo* pCConnectInfo = m_listCConnectInfo.GetAt(pos);
			m_listCConnectInfo.RemoveAt(pos);
	...
	void		RemoveAll()
	{
		...
		POSITION pos = m_listCConnectInfo.GetHeadPosition();
		while(pos)
		{
			CConnectInfo* pCConnectInfo = m_listCConnectInfo.GetNext(pos);
			SAFE_DELETE(pCConnectInfo);
		}

		//Empty the list...
		m_listCConnectInfo.RemoveAll();
	}

	POSITION	FindConnectInfo(LPCWSTR pwszInitString)
	{
		POSITION pos = m_listCConnectInfo.GetHeadPosition();
		...
		while(pos)
		{
			...
			if(pwszInitString)
			{
				if(wcscmp(pwszInitString, pCConnectInfo->GetInitString())==0)
					return posSave;
			}
			else
			{		
				//Maybe they are both NULL
				if(!pCConnectInfo->GetInitString())
					return posSave;
	...
	void		SetConnectInfo(CConnectInfo* pCConnectInfo)
	{
		ASSERT(pCConnectInfo);
		POSITION pos = FindConnectInfo(pCConnectInfo->GetInitString());

		if(pos)
		{
			//If the InitString already exists, to improve performance and to have a
			//list completely sorted by latest access time (ie: a stack) we remove
			//the current occurance and add it to the head (below)...
			Remove(pos);
		}
		
		//Now store the ConnectInfo in the Cache
		m_listCConnectInfo.AddHead(pCConnectInfo);
	}


protected:
	CList<CConnectInfo*, CConnectInfo*> m_listCConnectInfo;
};


//Connection Caching
static CConnectCache	s_CConnectCache;
...
public:
	...
	void		SetUDLInfo(CUDLInfo* pCUDLInfo, ULONG ulDevCacheSize)
	{
		...
		else
		{
			...
			//See if we are in synch with dev
			if(GetCurrentSize()+1 != ulDevCacheSize)
			{
				TWARNING("Cache Size out of synch Dev: " << ulDevCacheSize << " Internal: " << GetCurrentSize()+1);
			
				//Try to self correct ourselves...
				while(GetCurrentSize()+1 > ulDevCacheSize && !m_listCUDLInfo.IsEmpty())
				{
					DWORD dwMin = -1;
					POSITION posMin = NULL;
					
					//Find the min item and remove it...
					POSITION pos = m_listCUDLInfo.GetHeadPosition();
					while(pos)
					{
						POSITION posSave = pos;	//::GetNext moves the position
						CUDLInfo* pCUDLExpired = m_listCUDLInfo.GetNext(pos);
						if(pCUDLExpired->GetAccessID() < dwMin)
						{
							posMin = posSave;		
							dwMin  = pCUDLExpired->GetAccessID();
						}
					}

					if(posMin)
					{
						CUDLInfo* pCUDLInfo = m_listCUDLInfo.GetAt(posMin);
						m_listCUDLInfo.RemoveAt(posMin);
						SAFE_DELETE(pCUDLInfo);
					}
				}
			}
...
public:
	...
	virtual BOOL	CompareProperties(IUnknown* pDataSource, ULONG cPropSets, DBPROPSET* rgPropSets);
	virtual BOOL	CompareProperties(IUnknown* pDataSource, IUnknown* pDataSource2);
...
BOOL CDBInit::VerifyInitString(IUnknown* pDataSource, WCHAR* pwszInitString, BOOL fReturnedString, BOOL fInsideThreads)
{
	...
	for(i=0; i<cPropSets; i++)
	{
		...
		for(ULONG j=0; j<pPropSet->cProperties; j++)
		{
			...
			{
				TERROR("Missing Property Information for PropertyID = " << pProp->dwPropertyID);
				QTESTC(FALSE);
			}
...
BOOL CDBInit::VerifyGetDataSource(DBPROPID dwPropertyID, GUID guidPropertySet, DBTYPE wType, void* pValue, REFIID riid, IUnknown** ppDataSource, WCHAR* pwszInitString)
{
	...
	if(hr==S_OK)
	{
		...
		else
		{
			...
			TEST3C_(hr, DB_E_ERRORSOCCURRED, DB_SEC_E_AUTH_FAILED, E_FAIL);
		}
...
//
////////////////////////////////////////////////////////////////////////////
BOOL CDBInit::CompareProperties(IUnknown* pDataSource, IUnknown* pDataSource2)
{
	ASSERT(pDataSource);
	ASSERT(pDataSource2);
	TBEGIN
	...
	//GetProperties
	QTESTC(EqualPropSets(cPropSets, rgPropSets, cPropSets2, rgPropSets2));
...
//
////////////////////////////////////////////////////////////////////////////
BOOL CDBInit::CompareProperties(IUnknown* pDataSource, ULONG cPropSets, DBPROPSET* rgPropSets)
{
	ASSERT(pDataSource);
	TBEGIN
	HRESULT hr = S_OK;
...
BEG_TEST_CASE(TCGetDataSource, CDBInit, L"Test IDataInitialize::GetDataSource")
	...
	TEST_VARIATION(85, 		L"Strings - Reserved SemiColon - Semicolon following keyword Data Source;")
...
{ 
	...
	TESTC_(pIClassFactory->QueryInterface(IID_IUnknown,			NULL), E_INVALIDARG);
	TESTC_(pIClassFactory->QueryInterface(IID_IClassFactory,	NULL), E_INVALIDARG);
...
{ 
	...
	//QI Invalid Combinations
	TESTC_(pIDataInitialize->QueryInterface(IID_IUnknown, NULL),		E_INVALIDARG);
	TESTC_(pIDataInitialize->QueryInterface(IID_IDataInitialize, NULL),	E_INVALIDARG);
...
{ 
	...
	for(i=0; i<cPropInfoSets; i++)
	{
		...
		for(ULONG j=0; j<pPropInfoSet->cPropertyInfos; j++)
		{
			...
			//Verify this Property
			TESTC(VerifyGetDataSource(pPropInfo->dwPropertyID, pPropInfoSet->guidPropertySet, pPropInfo->vtType));
		}
	}
...
{ 
	...
	HRESULT hrExpected = CoCreateInstance(CLSID_MSDASQL, NULL, CLSCTX_INPROC_SERVER, IID_IDBInitialize, (void**)&pIDBInitialize);

	//Test InitString
	//GetDataSource with no Provider Keyword and *ppDataSource = NULL on input
	TESTC_(GetDataSource(NULL, L"", IID_IUnknown, &pIUnknown), hrExpected);
	if(pIUnknown)
	{
#ifdef _WIN64
...
//		iSettableProps++;
	...
	//Test InitString
	TESTC_(GetDataSource(NULL, pwszInitString, IID_IUnknown, &pIUnknown), iSettableProps ? DB_E_ERRORSOCCURRED : S_OK);
...
{ 
	TBEGIN
	IUnknown* pIUnknown = NULL;
	DBPROPID dwPropertyID = DBPROP_INIT_LOCATION;

	//Create InitString
	WCHAR* pwszLocation		= L"Data Source=localAccess; User ID=odbc; Password = odbc;";
	WCHAR* pwszBonusStuff	= L" This;IsAkeyword = my value";
	WCHAR* pwszInitString	= CreateString(L"Provider =%s;%s", GetProgID(), pwszBonusStuff);
	
	//DBPROP_INIT_LOCATION
	if(SettableProperty(dwPropertyID, DBPROPSET_DBINIT, pDBInitSC()))
		AppendString(&pwszInitString, L"Location = \"Data Source=localAccess; User ID=odbc; Password = odbc;\"");
	...
	if(fProviderString)
	{
		...
		TESTC(VerifyDataSource(pIUnknown, dwPropertyID, DBPROPSET_DBINIT, DBTYPE_BSTR, pwszLocation));

		//Verify the Bonus stuff was placed in the Provider String
		TESTC(VerifyDataSource(pIUnknown, DBPROP_INIT_PROVIDERSTRING, DBPROPSET_DBINIT,	DBTYPE_BSTR, pwszBonusStuff));
	}
	else
	{
		//ProviderString is not supported, so trying to put the extra
		//untokenized characters in the extended properties should fail...
...
{ 
	...
	//Since "Extended Properties; Provider" is not a valid keyword
	//(An equal sign ends the keyword), the entire string should be placed 
...
{ 
	...
	GetWindowsDirectoryA(szBuffer, MAX_QUERY_LEN);
...
{ 
	...
	//Create Locale Object
	CLocaleInfo LocaleInfo( GetUserDefaultLCID() );
	LocaleInfo.MakeUnicodeIntlString(wszFileName, _MAX_FNAME - strlen(szBuffer));
...
{ 
	...
	//Make sure the cache is full, (if not already)
	FullDevUDLCache();
	ULONG i,cMaxSize = s_CUDLCache.GetCurrentSize();

	//Since most of the rest of the test does a lot with unique files names which 
	//really tests full caches, and newly added items.  There is also reuse, but
	//not a must true "pegging" to verify internal data structures.  To do this will 
	//will requests only UDLs from the cache, but all over the place, forward, backward,
	//out of order, reverse, end, head, etc...

	//Retrieve in-order
	for(i=0; i<cMaxSize; i++)
		TESTC(VerifyLoadString(s_CUDLCache[i]->GetFileName(), s_CUDLCache[i]->GetInitString())); 

	//Retrieve in Reverse order
	for(i=0; i<cMaxSize; i++)
		TESTC(VerifyLoadString(s_CUDLCache[cMaxSize-i-1]->GetFileName(), s_CUDLCache[cMaxSize-i-1]->GetInitString())); 

	//Retrieve in-order (every other one)
	for(i=0; i<cMaxSize; i+=2)
		TESTC(VerifyLoadString(s_CUDLCache[i]->GetFileName(), s_CUDLCache[i]->GetInitString())); 

	//Retrieve in Reverse order (every other one)
	for(i=0; i<cMaxSize; i+=2)
		TESTC(VerifyLoadString(s_CUDLCache[cMaxSize-i-1]->GetFileName(), s_CUDLCache[cMaxSize-i-1]->GetInitString())); 

	//Retrieve Limts
	TESTC(VerifyLoadString(s_CUDLCache[0]->GetFileName(), s_CUDLCache[0]->GetInitString())); 
	TESTC(VerifyLoadString(s_CUDLCache[cMaxSize-1]->GetFileName(), s_CUDLCache[cMaxSize-1]->GetInitString())); 
	TESTC(VerifyLoadString(s_CUDLCache[cMaxSize/2]->GetFileName(), s_CUDLCache[cMaxSize/2]->GetInitString())); 
...
{ 
	...
	//Delegate to our thread function, which already does this...
	Thread_RandomLoadString(&T1Arg);
...
{ 
	...
	//Create Threads
	CREATE_THREADS(Thread_RandomLoadString, &T1Arg);
	START_THREADS();
	END_THREADS();	
...
{ 
	...
	//Otherwsie they will all end up in the same location...
	THREADARG T1Arg = { this, pCUDLInfo, (void*)STG_E_SHAREVIOLATION };
	THREADARG T2Arg = { this, pCUDLInfo, (void*)STG_E_SHAREVIOLATION };
...
{ 
	...
	//QI Invalid Combinations
	TESTC_(pIDBPromptInitialize->QueryInterface(IID_IUnknown, NULL),			E_INVALIDARG);
	TESTC_(pIDBPromptInitialize->QueryInterface(IID_IDBPromptInitialize, NULL),	E_INVALIDARG);
...
{ 
	...
	static const cSourceType = NUMELEM(rgSourceType);

	static const DBSOURCETYPE rgInvalidSourceType[] = { DBSOURCETYPE_DATASOURCE, DBSOURCETYPE_ENUMERATOR };
	static const cInvalidSourceType = NUMELEM(rgInvalidSourceType);
...
{ 
	...
	//GetDataSource
	TESTC_(GetDataSource(NULL, m_pwszInitString, IID_IDBInitialize, (IUnknown**)&pIDBInitialize), S_OK);
