...
// Track whether running against Kagera
BOOL	g_fKagera;

// Initially use Sql Server provider specific guid and prop value
GUID	g_guidTableStatistics = DBSCHEMA_TABLE_STATISTICS;
...
)
{
	...
	else if(IsEqualGUID(g_guidTableStatistics,guid))
...
ULONG CSchemaTest::NumberofRestrictions(GUID schema)
{
	...
	if(IsEqualGUID(schema,DBSCHEMA_KEY_COLUMN_USAGE) ||
		...
		IsEqualGUID(schema,g_guidTableStatistics))
		return 127;
...
)
{
	...
	else if(IsEqualGUID(g_guidTableStatistics,m_guid))
...
)
{
		...
		else if(IsEqualGUID(m_guid,g_guidTableStatistics))
...
BOOL CSchemaTest::Find_Table_Statistics_Restrictions()
{
	...
	if(!IsSchemaSupported(g_guidTableStatistics))
		return FALSE;
	...
	// if this is a schema I want then get information for schema
	if(GetSchemaInfo(SPECIFIC,0,g_guidTableStatistics))
	{
...
ULONG CSchemaTest::SchemaCatalogRestriction(ULONG ulIndex)
{
	...
	if (guidSchema == DBSCHEMA_ASSERTIONS ||
		...
		guidSchema == g_guidTableStatistics ||
	...
	DBBINDING * pStartBind, DBBINDING * pEndBind, LPBYTE pDataRows, DBLENGTH cbRowSize)
{
	...
	switch (eCardinality)
	{
		...
		{
			ULONG cchWhere = wcslen(wszWHERE); // Length of " where "
			ULONG cchColName = wcslen(m_prgColInfo[ulColIndex].pwszName);
	...
	if (cParams)
	{
		...
		pICommand = m_pTable->m_pICommand;
...
BOOL ModuleInit(CThisTestModule * pThisTestModule)
{
	...
	if (ModuleCreateDBSession(pThisTestModule))
	{
		...
		GetProperty(DBPROP_PROVIDERNAME, DBPROPSET_DATASOURCEINFO, pThisTestModule->m_pIUnknown, &pwszProviderName);
		if (pwszProviderName && !wcscmp(pwszProviderName, L"MSDASQL.DLL"))
			g_fKagera=TRUE;
...
protected:
	...
	BOOL VerifyRestriction(DBORDINAL iOrdinal, DATA * pColumn);
...
BOOL CHelper::Init(CSchemaTest * pThis)
{
	...
	// Insert any rows into this schema that are needed
	TESTC(InsertRows())

	TESTC(GetRestrictions(0));

	// Call the GetProperty for the UserName, which matches the schema name
	GetProperty(DBPROP_USERNAME, DBPROPSET_DATASOURCEINFO, pThis->m_pIDBInitialize, &m_pwszSchemaName);

	// Get DBPROP_NULLCOLLATION for use in comparing sort of NULL values
	GetProperty(DBPROP_NULLCOLLATION, DBPROPSET_DATASOURCEINFO, pThis->m_pIDBInitialize, &m_ulNullCollation);
...
BOOL CHelper::SetRestriction(ULONG iRestrict, LPVOID pValue)
{
	...
	if (COMPARE(iRestrict > 0 && iRestrict <= m_pSchemaInfo->cRestrictions, TRUE))
	{
		iOrdinal = m_pSchemaInfo->pulRestrictions[iRestrict];
...
BOOL CHelper::VerifyLength(DBORDINAL iBind, DATA * pColumn)
{
	...
	ULONG ulLength = GetDBTypeSize(wType);
	BOOL fResult = TRUE;
...
BOOL CHelper::VerifyValue(DBORDINAL iOrdinal, DATA * pColumn)
{
	...
	for (ULONG iVal = 0; iVal < cValidVals; iVal++)
	{
		...
		USHORT cbBackEnd = (USHORT)ulTypeSize, cbConsumer = (USHORT)ulTypeSize;
...
{
	// Need to insert at least two rows into schema for sorting test
	odtLog << L"InsertRows not implemented.\n";
	return TRUE;
}
...
BOOL CColumnsHelper::VerifyColumn(DBORDINAL iOrdinal, DATA * pColumn)
{
	...
	//					case DBTYPE_BOOL:  // We assume BOOL is not a "bit" column per spec.
			...
			case COLS_DOMAIN_NAME:
				...
				else if (!COMPARE(ulStatus, DBSTATUS_S_ISNULL) && m_prgColInfo)
					odtLog << m_pwszTableName << L": " << m_prgColInfo[iIndex].pwszName << L": " <<
						L"Invalid status returned.\n";
				break;
...
public:
	...
	CProcedureParametersHelper(void) {m_guidSchema = DBSCHEMA_PROCEDURE_PARAMETERS;}
};
...
public:
	...
	CProviderTypesHelper(void) {m_guidSchema = DBSCHEMA_PROVIDER_TYPES;}
};
...
public:
	...
	CTableStatisticsHelper(void) {m_guidSchema = DBSCHEMA_TABLE_STATISTICS;}
			...
			pColumn->sStatus == DBSTATUS_S_OK)
		{
			...
			if (m_prgColInfo)
			{
				...
				else
				{
					...
					// Note test bug here Cardinality() doesn't include BLOB columns.
...
int CCommon::Variation_11()
{
	...
	for(ulIndexProperties=0;ulIndexProperties<cUPDATE_PROPERTIES;ulIndexProperties++)
	{
			...
			(GetProperty(rgUpdateProperties[ulIndexProperties],DBPROPSET_ROWSET,m_pTable->m_pICommand))))
		{
			...
			{
				COMPARE(VerifyPropStatus(cPropSet, pPropSet, rgUpdateProperties[ulIndexProperties],
					DBPROPSET_ROWSET, DBPROPSTATUS_CONFLICTING), TRUE);
			}
...
int CCommon::Variation_12()
{
	...
	{
		odtLog << L"IRowsetInfo is not supported\n";
		return TEST_SKIPPED;
	}
	...
	TESTC(VerifyPropStatus(cPropSet, pPropSet, DBPROP_IRowsetInfo, DBPROPSET_ROWSET, DBPROPSTATUS_CONFLICTING))
	TESTC(VerifyPropStatus(cPropSet, pPropSet, DBPROP_IRowsetChange, DBPROPSET_ROWSET, DBPROPSTATUS_CONFLICTING))
...
int CCommon::Variation_13()
{
	...
	if (S_OK == hr)
	{
		TESTC(VerifyPropStatus(cPropSet, pPropSet, DBPROP_BOOKMARKS, DBPROPSET_ROWSET, DBPROPSTATUS_OK));
	}
	else
	{
		TESTC_(hr, DB_E_ERRORSOCCURRED);
		TESTC(VerifyPropStatus(cPropSet, pPropSet, DBPROP_BOOKMARKS, DBPROPSET_ROWSET, DBPROPSTATUS_NOTSETTABLE));
	}
...
int CCommon::Variation_22()
{
	...
	TESTC(VerifyPropStatus(cPropSet, pPropSet, DBPROP_IColumnsRowset, DBPROPSET_ROWSET, DBPROPSTATUS_OK));
...
{ 
	...
	for(ULONG iProp=0; iProp < cPropInfos; iProp++)
	{
		...
		else
		{
			...
			// And finally, if this is DBPROP_UPDATABILITY itself, we need to allow
		...
		COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropertyID,
			DBPROPSET_ROWSET, dwPropStatusExpected), TRUE);
	...
	for(ULONG iProp=0; iProp < cPropInfos; iProp++)
	{
		for (ULONG jProp = 0; jProp < cPropInfos; jProp++)
		{
			...
			if (S_OK == hr)
			{
				...
				fStatusOK &= COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropertyID,
					DBPROPSET_ROWSET, DBPROPSTATUS_OK), TRUE);
				fStatusOK &= COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropIDOpt,
					DBPROPSET_ROWSET, DBPROPSTATUS_OK), TRUE);
			}
			else if (DB_S_ERRORSOCCURRED == hr)
			{
				...
				if (pPropSet[0].rgProperties[1].dwStatus != DBPROPSTATUS_NOTSUPPORTED &&
					...
					fStatusOK &= COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropIDOpt,
						DBPROPSET_ROWSET, DBPROPSTATUS_NOTSET), TRUE);
			}
			else
			{
				...
				fStatusOK &= COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropertyID,
					DBPROPSET_ROWSET, dwStatusExpected), TRUE);
...
//			odtLog << pPropInfos[iProp].pwszDescription << L"\n";
			...
			dwPropStatusExpected = DBPROPSTATUS_NOTSUPPORTED;
			COMPARE(VerifyPropStatus(cPropSet, pPropSet, dwPropertyID,
				m_rgDBPROPINFOSET[iInfoSet].guidPropertySet, dwPropStatusExpected), TRUE);
...
int CGetSchema::Variation_3()
{
	...
	// test method
	// Spec change.  Passing NULL prgRestrictionSupport is now a success case
