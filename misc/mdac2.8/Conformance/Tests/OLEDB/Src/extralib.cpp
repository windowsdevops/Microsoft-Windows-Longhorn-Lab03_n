...
HRESULT SetRegEntry(HKEY hRootKey, WCHAR* pwszKeyName, WCHAR* pwszValueName, WCHAR* pwszValue)
{
	...
	else
	{
		...
		return SetRegEntry(hRootKey, pwszKeyName, pwszValueName, pwszValue ? szBuffer : "", pwszValue ? (strlen(szBuffer)+1)*sizeof(CHAR) : sizeof(CHAR), REG_SZ);
	}
}
...
HRESULT AppendString(WCHAR** ppwszOldString, WCHAR* pwszAppend)
{
	...
	ULONG ulNewLen = wcslen(*ppwszOldString) + wcslen(pwszAppend);
...
//
////////////////////////////////////////////////////////////////
BOOL EqualProperty(DBPROP* pProp, DBPROP* pProp2)
{
	TBEGIN
	ASSERT(pProp);
	ASSERT(pProp2);
...
//
////////////////////////////////////////////////////////////////
BOOL EqualPropSets(ULONG cPropSets, DBPROPSET* rgPropSets, ULONG cPropSets2, DBPROPSET* rgPropSets2)
{
	...
	for(i=0; i<cPropSets; i++)
	{
		...
		for(j=0; j<pPropSet->cProperties; j++)
		{
			...
			QTESTC(EqualProperty(pProp, pProp2));
		}
	}
...
HRESULT CRowset::ReleaseRows(DBCOUNTITEM cRows, HROW* rghRow, DBREFCOUNT* rgRefCounts, DBROWSTATUS* rgRowStatus)
{
	...
	//ReleaseRows
	HRESULT hr = pIRowset()->ReleaseRows(cRows, rghRow, NULL, rgRefCounts, rgRowStatus);
