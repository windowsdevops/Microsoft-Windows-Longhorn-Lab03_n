...
HRESULT CAsynchDSO::Initialize(DWORD dwPropVal)
{
	...
	if(dwPropVal == DBPROPVAL_ASYNCH_INITIALIZE)
		SetSettableProperty(DBPROP_INIT_ASYNCH, DBPROPSET_DBINIT, (void*)(ULONG_PTR)dwPropVal, DBTYPE_I4);
	else
...
HRESULT CAsynchRowset::CreateRowset(DWORD dwPropVal)
{
	TBEGIN
	HRESULT hr = S_OK;
	...
	SetSettableProperty(DBPROP_ROWSET_ASYNCH, DBPROPSET_ROWSET, (void*)(ULONG_PTR)dwPropVal, DBTYPE_I4);
...
public:
	...
	//members
	virtual BOOL	InitSources(DWORD dwPropVal = DBPROPVAL_ASYNCH_INITIALIZE);
...
BOOL CTestNotify::Terminate()
{
	...
	for(DBCOUNTITEM i=0; i<m_vectSources.GetCount(); i++) 
...
BOOL CTestNotify::InitSources(DWORD dwPropVal) 
{
	TBEGIN
	...
	for(ULONG i=0; i<3; i++)
	{
		...
		switch(m_dwTestCaseParam)
		{
			...
			case COMMAND_INTERFACE:
				pCSource = new CAsynchRowset(SELECT_ORDERBYNUMERIC);
				break;
...
BOOL CTestNotify::AdviseAll()
{
	...
	for(ULONG iListener=0, index=0; iListener<m_vectListeners.GetCount(); iListener++)
	{
		for(ULONG iSource=0; iSource<m_vectSources.GetCount(); iSource++, index++)
		{
...
BOOL CTestNotify::VerifyAllConnections(DWORD dwResult)
{
	...
	{
		for(ULONG iSource=0; iSource<m_vectSources.GetCount(); iSource++)
		{
...
{ 
	...
	for(iSource=0; iSource<m_vectSources.GetCount(); iSource++)
	{
		...
		//Make sure no other Listeners were notified
		for(ULONG iListener=0; iListener<m_vectListeners.GetCount(); iListener++)
		{
...
{ 
	...
	TESTC_PROVIDER(pCSource(0)->GetObjectType() == ROWSET_INTERFACE);
