...
BOOL COraproc::VerifyProcColumns()
{
	...
	V_BSTR(&(rgRestrict[2])) = SysAllocStringLen(NULL, wcslen(m_pwszPackageName)+wcslen(m_pwszRefProcName)+1+1);
...
public:
	...
	// {{ TCW_TESTVARS()
	// @cmember Char Output parameter, 32k size, no prepare, no setparaminfo
	int Variation_1();
	// }} TCW_TESTVARS_END
} ;
...
BEG_TEST_CASE(PLSQL_Types, COraproc, L"Test all PL/SQL Types")
	TEST_VARIATION(1, 		L"Char Output parameter, 32k size, no prepare, no setparaminfo")
END_TEST_CASE()
#undef THE_CLASS
// }} TCW_TESTCASE_END
// }} TCW_TEST_CASE_MAP_END
...
// {{ TCW_VAR_PROTOTYPE(1)
//*-----------------------------------------------------------------------
// @mfunc Char Output parameter, 32k size, no prepare, no setparaminfo
//
// @rdesc TEST_PASS or TEST_FAIL 
//
...
{ 
	...
	TESTC_(hr=Execute(	L"create or replace procedure sp_bigchar( par_out_bigchar OUT CHAR)  as"
	                    ...
	                    L" begin   "
						L"	tempC := 'a';"
						L"	par_out_bigchar :=  RPAD(tempC, 32000, '1' ); "
						L"end;" ), S_OK);

	MakeBinding(1, DBPARAMIO_OUTPUT, DBTYPE_WSTR, NULL, 0, 0, L"CHAR", DBTYPE_STR, 32000);
