...
int CExecute_Rowset::Variation_50()
{
	...
	hr = cTable.CreateTable(0, 0, m_pTable->GetTableName());
...
*/
	...
	V_I4(&dbProp[0].vValue) = 1;// set to 2 in ADO's bug
	...
	CLEANUP(!GetCommand(IID_ICommand,PPI &pICmd2,NULL))
	CLEANUP(FAILED(SetCommandText(m_pIMalloc,pICmd2,m_pTable,g_Table2->GetTableName(),
		eSELECT,SELECT_LEFTOUTERJOIN,NULL,NULL,NULL,g_Table2)));
	
	m_hr = Execute(pICmd2,PPI &pIUnknown1);
...
*/
	...
	dbProp[0].colid = DB_NULLID;
	dbProp[0].vValue.vt = VT_I4;
	V_I4(&dbProp[0].vValue) = 2;// set to 2 in ADO's bug
...
{ 
	...
	{
		for (ULONG iNewIndex = 1; iNewIndex <= g_Table2->CountColumnsOnTable();
			iNewIndex++)
		{
				...
				if (phRows)
				{
					for (iRow = 0; iRow < cRowsObtained; iRow++)
					{
						...
						for (jRow = 0; jRow < cRowsObtained; jRow++)
						{
							...
							{
								TESTC(!CompareBuffer(pData1, pData2, cBindings, pBindings, NULL, FALSE, FALSE, COMPARE_ONLY, FALSE, 0, NULL, TRUE))
							}
...
//	for(ulIndex=0; ulIndex<1; ulIndex++)
	{
		...
		TESTC_(GetAccessorAndBindings(pIUnk, 
			...
			NO_BLOB_COLS | BLOB_IID_IUNKNOWN, NULL),S_OK)
