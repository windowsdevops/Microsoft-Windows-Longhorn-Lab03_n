...
{	
	HROW			hRow		= NULL;
	HACCESSOR		hAccessor	= NULL;
	DBBINDSTATUS	rgDBBindStatus[50];	
	void			*pData		= NULL;
	BOOL			fTestPass	= TRUE;
	...
	//create an accessor
	if(FAILED(m_hr=m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA,m_cBinding, m_rgBinding, 0,&hAccessor,rgDBBindStatus)))
	{
		COMPARE(m_hr, DB_E_ERRORSOCCURRED);
		COMPARE(rgDBBindStatus[m_cBinding-1],DBBINDSTATUS_BADORDINAL);
		fTestPass=TRUE;
		hRow=NULL;
		goto CLEANUP;
	}
