...
int Boundary::Variation_9()
{
	...
	TESTC_PROVIDER(GetRowsetAndAccessor(SELECT_ORDERBYNUMERIC,1,m_rgPropertyIDs,0,NULL,ON_COMMAND_ACCESSOR,
		TRUE,DBACCESSOR_PARAMETERDATA,DBPART_VALUE|DBPART_STATUS|DBPART_LENGTH, UPDATEABLE_COLS_BOUND,
	...
	if( hAccessor2 )
		TEST3C_(SetData(hRow[0],hAccessor2,m_pData),DB_E_BADACCESSORTYPE,DB_E_ERRORSOCCURRED,DB_E_BADACCESSORHANDLE);
	if( hAccessor3 )
...
{ 
	...
	while (S_OK==(hr = pTypesRowset->GetNextRows(NULL,0,1,&cRows, &pHRow)) || hr == DB_S_ENDOFROWSET )
	{
		...
		if(	wcsstr(((WCHAR*)((BYTE*)pData + m_rgBinding[cTypeCol].obValue)), L"SYSTEM TABLE")	||
			wcsstr(((WCHAR*)((BYTE*)pData + m_rgBinding[cTypeCol].obValue)), L"SYNONYM"))
		{
...
//			if(!COMPARE(DBSTATUS_S_OK, *(DBSTATUS *)((DWORD)pData+m_rgBinding[0].obStatus)))
		...
		{
			TESTC_(hr, DB_E_CANTCONVERTVALUE);
		}
	}
...
//	*(ULONG *)((DWORD)pData+m_rgBinding[1].obLength)=m_rgBinding[1].cbMaxLen+1;
	...
	TEST3C_(hr,DB_E_ERRORSOCCURRED,DB_S_ERRORSOCCURRED,DB_E_CANTCONVERTVALUE);
...
int Invalid_Keyset_Cursor_Buffered::Variation_12()
{
		...
		)
	{
		...
		COMPARE(pDBRowStatus[3],DBROWSTATUS_S_OK);//no hole from  delete, should be new row
