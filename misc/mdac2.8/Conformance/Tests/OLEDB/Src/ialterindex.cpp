			...
			for (ULONG iRow=0; iRow < cRows; iRow++)
			{
				...
				for (ULONG iPropSet = 0; iPropSet < cIndexPropSets; iPropSet++)
				{
					...
					for (ULONG iProp = 0; iProp < rgIndexPropSets[iPropSet].cProperties; iProp++)
					{
						...
						for (ULONG iMap = 0; iMap < NUMELEM(IndexProperties); iMap++)
...
//METHODS...
	...
	WCHAR* BuildValidName(ULONG length, WCHAR* pattern);

	WCHAR* BuildInvalidName(ULONG length, WCHAR* pattern, WCHAR* invchars);
...
//
WCHAR* CAlterIndex::BuildValidName(ULONG length, WCHAR* pattern)
{
	WCHAR*	pwszBuffer;
	ULONG	cLen = wcslen(pattern), i;
...
//
WCHAR* CAlterIndex::BuildInvalidName(ULONG length, WCHAR* pattern, WCHAR* invchars)
{
	WCHAR*	pwszBuffer;
	ULONG	cLen = wcslen(pattern), i, cInvLen=wcslen(invchars);
...
{ 
	...
	ULONG		iChar;
	HRESULT		hr = E_FAIL;
	ULONG		cchName = wcslen(m_pIndexID->uName.pwszName);
	WCHAR		wszValidChar[2] = L"A";
	WCHAR		*pwszValidChars = L"`1234567890-=~!@#$%^&*()_+[]\\;',./{}|:\"<>?";
	ULONG		cchValid = wcslen(pwszValidChars);
...
{ 
	...
	for (iThrd = 0; iThrd < MAX_THREADS; iThrd++)
	{
		...
		TArgs[iThrd].pArg1 = &(m_pTable->GetTableID()); // Table to alter
...
{ 
	...
	ULONG cCols = 1;
	DBINDEXCOLUMNDESC	*pIndexColumnDesc = NULL;
	DBID * pTableFirst = &(m_pTable->GetTableID());
	DBID * pTableSecond = &(m_pNoNullTable->GetTableID());
...
{ 
	...
	ULONG cCols = 1;
	DBINDEXCOLUMNDESC	*pIndexColumnDesc = NULL;
	DBID * pTable = &(m_pNoNullTable->GetTableID());
	DBID * pIndexID = NULL;
	CDBID dbid(DBKIND_NAME, L"Idx00001");
...
{ 
	...
	DBID * pTableID = &(m_pNoNullTable->GetTableID());
...
{ 
	...
	DBID *			pdbidTable		= &(m_pNoNullTable->GetTableID());
	IRowset *		pIRowset = NULL;
	HRESULT			hr = E_FAIL;
...
{ 
	...
	ULONG		iChar;
	HRESULT		hr = E_FAIL;
	ULONG		cchInvalid = wcslen(m_pwszInvalidTableChars);
	ULONG		cchName = wcslen(m_pTable->GetTableID().uName.pwszName);
...
{ 
	...
	ULONG		iChar;
	HRESULT		hr = E_FAIL;
	ULONG		cchInvalid = wcslen(m_pwszInvalidIndexChars);
	ULONG		cchName = wcslen(m_pIndexID->uName.pwszName);
	WCHAR		wszInvalidChar[2] = L"A";
...
{ 
	...
	DBID		*	pdbidTable		= &(m_pTable->GetTableID());
	IRowsetIndex *	pIRowsetIndex	= NULL;
...
{ 
	...
	DBID *		pTableID = &(m_pTable->GetTableID());
	CIndexInfo  IndexInfo;
	DBINDEXCOLUMNDESC	*pIndexColumnDesc = NULL;
