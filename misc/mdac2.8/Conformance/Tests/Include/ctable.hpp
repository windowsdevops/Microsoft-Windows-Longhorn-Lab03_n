...
//
//-----------------------------------------------------------------------------
class DLL_EXPORT CTable : public CSchema
{
	// @access Private
	private:	 
	...
	inline void AddRow(DBCOUNTITEM ulRow=1){m_ulRows += ulRow; m_ulNextRow += ulRow;};
	...
	inline void SubtractRow(DBCOUNTITEM ulRow=1){if(m_ulRows>0) m_ulRows -= ulRow; if(m_ulNextRow>0) m_ulNextRow -= ulRow;};
	...
	inline DBCOUNTITEM GetRowsOnCTable(void){return m_ulRows;};
	...
	inline WCHAR * GetTableName(void){return m_TableID.uName.pwszName;};

	// @cmember,mfunc Returns ViewName, client should not alter pointer.
	inline WCHAR * GetViewName(void){return m_pwszViewName;};	
		
	// @cmember,mfunc Returns TableID
	inline DBID GetTableID(void){return m_TableID;};	

	// @cmember,mfunc Sets TableID
	void SetTableID(DBID d);

	// @cmember,mfunc Returns m_fHasAuto
	inline BOOL GetHasAuto(void){
		return m_fHasAuto;}	

	// @cmember,mfunc Sets m_fHasAuto
	inline void SetHasAuto(BOOL b){m_fHasAuto = b;}

	// @cmember,mfunc Returns m_fHasAuto
	inline BOOL GetHasUnique(void){
		return m_fHasUnique;}	

	// @cmember,mfunc Sets m_fHasAuto
	inline void SetHasUnique(BOOL b){m_fHasUnique = b;}

	// @cmember,mfunc Returns m_fHasAuto
	inline BOOL GetHasCLSID(void){
		return m_fHasCLSID;}	

	// @cmember,mfunc Sets m_fHasAuto
	inline void SetHasCLSID(BOOL b){m_fHasCLSID = b;}

	// @cmember,mfunc Returns m_fHasDefault
	inline BOOL GetHasDefault(void){
		return m_fHasDefault;}	

	// @cmember,mfunc Sets m_fHasDeffault
	inline void SetHasDefault(BOOL b){m_fHasDefault = b;}

	// @cmember,mfunc Returns IndexName, client should not alter pointer.
	inline WCHAR * GetIndexName(void){return m_pwszIndexName;}
	
	// @cmember,mfunc Returns Module Name, client should not alter pointer.
	inline WCHAR * GetModuleName(void);

	// @cmember,mfunc Returns next row number for insert statement.
	inline DBCOUNTITEM GetNextRowNumber(void){return m_ulNextRow;}

	// @cmember Flag telling if Commands are supported on the CTable object.
	inline BOOL GetCommandSupOnCTable(void) { return m_pIDBCreateCommand!=NULL;	}

	// @cmember,mfunc Sets I4 for SQL support, m_lSQLSupport.
	// @@parmopt [IN] SQL Support (Default = 0)
	inline void SetSQLSupport(LONG_PTR lSQLSupport=0)
				{m_lSQLSupport=lSQLSupport;}
	
	// @cmember,mfunc Returns I4 for SQL support, m_lSQLSupport.
	inline LONG_PTR GetSQLSupport(void){return m_lSQLSupport;}

	// @cmember Flag telling if the Provider is ReadOnly.
	inline BOOL GetIsProviderReadOnly(void){return m_fProviderReadOnly;}
