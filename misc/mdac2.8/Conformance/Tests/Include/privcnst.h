...
//
//////////////////////////////////////////////////////////////////////////////
#include "modcommon.hpp"	// Need to include first, so we can override any Macros
...
DLL_EXPORT inline BOOL GetUseIMallocSpy();
DLL_EXPORT inline BOOL SetUseIMallocSpy(BOOL fValue);

DLL_EXPORT inline BOOL  PrivlibValidate(HRESULT hrActual, HRESULT hrExpected, BOOL fTreatAsWarning = FALSE, WCHAR* pwszFileName = NULL, ULONG ulLine = 0);
DLL_EXPORT inline BOOL  PrivlibCompare(BOOL fEqual, BOOL fTreatAsWarning = FALSE, WCHAR* pwszFileName = NULL, ULONG ulLine = 0);
DLL_EXPORT inline DWORD IsWarning(HRESULT hrActual, HRESULT hrExpected);
...
//Allocation Routines...
#define PROVIDER_ALLOC(cb)				(GetUseIMallocSpy() ? LTMALLOCSPY((ULONG)(cb)) : CoTaskMemAlloc((ULONG)(cb)))  //64bit TODO - remove ULONG cast
#define PROVIDER_REALLOC(pv, cb)        (GetUseIMallocSpy() ? LTMREALLOCSPY(pv, (ULONG)(cb)) : CoTaskMemRealloc(pv, (ULONG)(cb)))  //64bit TODO - remove ULONG cast
